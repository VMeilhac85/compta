...
MARKETING_VERSION="${IOS_MARKETING_VERSION:-$(awk '/^MARKETING_VERSION = / { print $3 }' "$PROJECT_DIR/Config/Base.xcconfig")}"
BUILD_NUMBER="${IOS_BUILD_NUMBER:-$(awk '/^CURRENT_PROJECT_VERSION = / { print $3 }' "$PROJECT_DIR/Config/Base.xcconfig")}"
