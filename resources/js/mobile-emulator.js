...
import { mountTwoFactorChallenge, mountTwoFactorSettings } from './two-factor';
import { observeVisibleNotifications } from './mobile-notification-visibility';
...
if (root) {
    ...
    const restorableRoutes = new Set([
        ...
        'employee_documents', 'absences', 'absence_create', 'expenses', 'expense_create', 'expense_resume', 'expense_item_create',
        'mileage', 'mileage_create', 'mileage_submit', 'mileage_settings', 'payslip_latest',
        'sales_revenue', 'employee_mobile_access', 'reports', 'billing', 'report_create', 'contacts', 'messages',
        'task_create', 'user_settings', 'footer_settings', 'about', 'desktop',
    ]);
    const grhRoutes = new Set([
        'hr', 'hr_pending', 'employee_documents', 'absences', 'absence_create',
        'expenses', 'expense_create', 'expense_resume', 'expense_item_create', 'mileage', 'mileage_create',
    ...
    const salariedRestrictedRoutes = new Set([
        ...
        'notes', 'contacts', 'messages', 'support',
    ]);
    ...
    let mobileFactorSettings = null;
    let mobileFactorExpiryTimer = null;
    ...
    function clearMobileAccessSession({ preserveNavigation = false } = {}) {
        forgetMobileFactorSettings();
        ...
        if (iosNativeRuntime) {
            try { window.webkit?.messageHandlers?.secureSession?.postMessage({ action: 'clear', preserve_navigation: preserveNavigation }); } catch (_) { /* Trousseau déjà vide. */ }
        }
    }
    ...
    const state = {
        ...
        notificationOpenOperation: null,
        notificationVisibilityCleanup: null,
        notificationListSequence: 0,
        messageDrafts: {},
        ...
        footerNavigationEditable: false,
        footerNavigationDraftScope: '',
        footerNavigationDrafts: {},
        footerNavigationOperation: null,
        notificationUnreadCount: null,
        notificationBadgeScope: '',
        notificationBadgeStatus: 'idle',
        notificationBadgeCheckedAt: 0,
        notificationBadgeOperation: null,
        updateCheckedAtUtc: null,
        updateCheckError: '',
        ...
        hrPendingValidationCount: null,
        hrLastSections: storedPreferences.hrLastSections || {},
        hrPendingError: null,
        hrPendingValidations: null,
        homeQuickActionCatalog: [],
        homeQuickActionKeys: [],
        homeQuickActionInitialKeys: [],
        homeQuickActionDefaultKeys: [],
        homeQuickActionDrafts: {},
        homeQuickActionScope: '',
        homeAttentionItems: [],
        homeNews: null,
        homeNewsScope: '',
        homeNewsLoading: false,
        homeNewsError: '',
        homeNewsLastAttempt: 0,
        homeNewsRequestSequence: 0,
        homeNewsController: null,
        homeNewsTimer: null,
        homePreferences: null,
        homePreferencesScope: '',
        homePreferencesChecked: false,
        homePreferencesReady: false,
        homePreferencesLoading: false,
        homePreferencesSaving: false,
        homePreferencesError: '',
        homePreferencesRetryValue: null,
        homePreferencesSequence: 0,
        homePreferencesController: null,
        homeResumeAction: null,
        ...
        notificationPreferencesRefreshController: null,
    ...
    function sanitizeOfflinePayload(payload, kind = '') {
        if (kind === 'home' && payload?.data) {
            payload = { ...payload, data: {
                ...payload.data,
                resume_action: null,
                attention_items: (payload.data.attention_items || []).filter((item) =>
                    !String(item.key || '').startsWith('expense') && !/^expenses(?:\/|$)/.test(String(item.destination || ''))),
            } };
        }
    ...
    async function storeOfflineResponse(path, kind, payload) {
        ...
        const safePayload = sanitizeOfflinePayload(payload, kind);
    ...
    async function readOfflineResponse(path, kind) {
        ...
        const payload = sanitizeOfflinePayload(entry.payload, kind);
    ...
    function persistPreferences() {
        try {
            localStorage.setItem(preferencesStorageKey, JSON.stringify({
                ...
                hrLastSections: state.hrLastSections,
            }));
        } catch (_) {
    ...
    const documentScopeIcons = {
        recent: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 2a8 8 0 1 1 0 16 8 8 0 0 1 0-16Zm-1 3h2v5l4 2-1 2-5-3V7Z"/></svg>',
    ...
    quickActionIcons.documents = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 4H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-8l-2-2Zm10 14H4V6h5.17l2 2H20v10Z"/></svg>';
    quickActionIcons.tasks = quickActionIcons.task;
    quickActionIcons.hr = quickActionIcons.absence;
    quickActionIcons.reports = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2Zm0 16H5V5h14v14ZM7 12h2v5H7v-5Zm4-5h2v10h-2V7Zm4 3h2v7h-2v-7Z"/></svg>';
    quickActionIcons.messages = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 2H4a2 2 0 0 0-2 2v18l4-4h14a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2Zm0 14H5.17L4 17.17V4h16v12Z"/></svg>';
    quickActionIcons.last_payslip = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2Zm0 16H5V5h14v14ZM7 7h10v2H7V7Zm0 4h10v2H7v-2Zm0 4h7v2H7v-2Z"/></svg>';
    ...
    async function api(path, options = {}) {
        ...
        const offlineKind = trustedMobileApiTarget && options.offlineCache !== false
        ...
        if (options.expectedIdentityScope && options.expectedIdentityScope !== currentUploadIdentityScope()) {
            const error = new Error('Le contexte a changé. Reprenez cette action dans le compte courant.');
            error.code = 'mobile_context_changed';
            throw error;
        }
    ...
    function prepareMaisonPiloteRender() {
        stopNotificationVisibilityTracking();
    ...
    async function uploadFileToGed(
        ...
        if (!upload?.document?.id || !['completed', 'duplicate'].includes(String(upload.status))) {
            throw new Error('Le serveur n’a pas encore confirmé l’arrivée du document dans Documents.');
        }
        onProgress(100);
        return upload;
    }
    ...
    const displayedShareIntakeResults = new Set();
    function showMobileShareIntakeResult(result) {
        const received = Array.isArray(result?.received) ? result.received : [];
        const rejected = Array.isArray(result?.rejected) ? result.rejected : [];
        if (!rejected.length) return;
        const countLabel = (count, missing = false) => `${count} fichier${count === 1 ? '' : 's'} ${missing ? 'non ' : ''}reçu${count === 1 ? '' : 's'}`;
        const title = received.length ? 'Partage incomplet' : 'Partage impossible';
        app.querySelector('[data-mobile-share-intake-result]')?.remove();
        app.insertAdjacentHTML('beforeend', `<div class="mobile-app-dialog-backdrop" data-mobile-share-intake-result role="presentation"><section class="mobile-app-quick-actions-dialog" role="dialog" aria-modal="true" aria-labelledby="mobile-share-intake-title"><header><h3 id="mobile-share-intake-title">${title}</h3><button type="button" data-mobile-share-intake-close aria-label="Fermer">×</button></header><div class="mobile-app-quick-actions-dialog__body mobile-app-share-intake"><p>${countLabel(received.length)} - ${countLabel(rejected.length, true)}.</p><ul>${rejected.map((file) => `<li class="is-error"><strong>${escapeHtml(file.name || 'Fichier partagé')}</strong><span>${escapeHtml(file.reason || 'Ce fichier ne peut pas être lu.')}</span></li>`).join('')}${received.map((file) => `<li><span aria-hidden="true">✓</span> ${escapeHtml(file.name || 'Fichier partagé')}</li>`).join('')}</ul><p>Partagez de nouveau les fichiers non reçus depuis l’application d’origine.</p></div></section></div>`);
        app.querySelector('[data-mobile-share-intake-close]')?.focus({ preventScroll: true });
    }
    window.addEventListener('maisonpilote:share-intake-result', (event) => showMobileShareIntakeResult(event.detail));
    ...
    function normalizeNativeShareInbox(value) {
        ...
        (Array.isArray(value) ? value : []).slice(0, 50).forEach((candidate) => {
            ...
            batches.push({
                id,
                intakeResult: candidate?.intakeResult || candidate?.intake_result || null,
    ...
    async function processNativeShareFile(batch, file, dossierId, uploadContext) {
        ...
        try {
            ...
            updateUploadFeedback(feedbackId, {
                ...
                detail: `${file.displayName} a été ajouté à Documents.`,
                status: 'success',
                progress: 100,
            ...
            updateUploadFeedback(feedbackId, {
                title: 'Envoi du partage interrompu',
                detail: error?.message || 'Le document partagé n’a pas pu être transmis à Documents.',
                status: 'error',
            });
    ...
    async function completeCameraPhotoReturn(imageFiles) {
        ...
        try {
            ...
            updateUploadFeedback(id, {
                ...
                detail: `${file.name}${destination ? ` a été déposé dans ${destination}.` : ' a été ajouté à Documents.'}`,
                status: 'success',
                progress: 100,
            ...
            if (!controller.signal.aborted) {
                updateUploadFeedback(id, {
                    ...
                    detail: error?.message || 'Le document n’a pas pu être préparé ou transmis à Documents.',
                    status: 'error',
                });
                if (app.querySelector('[data-mobile-quick-creation="expense"]') && uploadContext.contextType === 'expense_report') {
                    await refreshQuickExpenseDetail(Number(uploadContext.contextId));
    ...
    async function completeExternalFileReturn(explicitFiles = null) {
        ...
        const results = await Promise.allSettled(feedbacks.map(async ({ id, file }) => {
            ...
            try {
                ...
                updateUploadFeedback(id, {
                    title: 'Envoi interrompu',
                    detail: error?.message || 'Le document n’a pas pu être transmis à Documents.',
                    status: 'error',
                });
                throw error;
            } finally {
    ...
    async function completeAuthenticatedStart() {
        ...
        window.MaisonPiloteNative?.navigation?.retry?.();
    ...
    async function login(form) {
        ...
        try {
            ...
            if (response.data?.two_factor_required) {
                renderMobileTwoFactor(response.data);
                return;
            }
            await acceptMobileLogin(response);
        } catch (failure) {
            ...
            if (!authenticatedBeforeAttempt) {
                ...
                if (iosRuntime) clearMobileAccessSession({ preserveNavigation: true });
    ...
    let disposeMobileTwoFactor = null;

    function renderMobileTwoFactor(challenge) {
        disposeMobileTwoFactor?.();
        prepareMaisonPiloteRender();
        app.innerHTML = `<div class="mobile-app-auth">${logoMarkup()}<div class="mobile-app-auth__heading"><h1>Vérifiez votre identité</h1></div><p>Votre mot de passe est validé. Une dernière étape protège votre compte.</p><div data-mobile-two-factor></div><button class="mobile-app-auth__browser" type="button" data-mobile-two-factor-back>Revenir à la connexion</button></div>`;
        const perform = (action, body) => iosRuntime
            ? api(`/auth/two-factor/${action}`, { method: 'POST', publicAuth: true, body })
            : controlRequest(`/admin/emulation-app/double-authentification/${action}`, { body });
        disposeMobileTwoFactor = mountTwoFactorChallenge(app.querySelector('[data-mobile-two-factor]'), {
            challenge,
            onSend: async (method) => (await perform('send', { challenge_token: challenge.challenge_token, method })).data,
            onVerify: async (method, code) => {
                const response = await perform('verify', { challenge_token: challenge.challenge_token, method, code });
                challenge.challenge_token = '';
                disposeMobileTwoFactor?.(); disposeMobileTwoFactor = null;
                await acceptMobileLogin(response);
            },
        });
        app.querySelector('[data-mobile-two-factor-back]').addEventListener('click', () => {
            challenge.challenge_token = ''; disposeMobileTwoFactor?.(); disposeMobileTwoFactor = null; renderLogin();
        });
    }

    async function acceptMobileLogin(response) {
    ...
    async function logout() {
        if (!await flushMobileNotes()) return false;
        stopHomeNewsRefresh();
        stopHomePreferenceRequests();
        state.homePreferences = null;
        stopMessagePolling();
        ...
        state.homeNews = null;
        state.homeNewsError = '';
        state.homeNewsLastAttempt = 0;
        state.homeQuickActionDrafts = {};
        state.homeQuickActionScope = '';
    ...
    function mobileNavigationCatalog() {
        return [
            ...
            ['billing', 'Facturation', '', 'Consulter les factures et devis émis'],
        ...
        ].filter(([route]) => (!isSalariedUser() || !['hr', 'notes', 'contacts', 'support'].includes(route))
            && (route !== 'documents' || can('documents.list'))
            && (route !== 'reports' || can('reports.generate') || can('reports.download'))
            && (route !== 'billing' || can('billing.read'))
            ...
            && (route !== 'notifications' || mobileNotificationsAllowed()));
    }
    ...
    function mobileNotificationsAllowed() {
        return (state.bootstrap?.capabilities || []).includes('notifications.list') || can('notifications.list');
    }

    function limitedFooterRoutes(routes) {
        const available = mobileNavigationCatalog().map(([route]) => route);
        const pins = [...new Set(routes)].filter((route) => available.includes(route) && !['home', 'more'].includes(route));
        return ['home', ...pins.slice(0, 3), 'more'].filter((route) => available.includes(route));
    }

    function defaultFooterRoutes() {
        const preferred = isSalariedUser()
            ? ['payslips', 'absences', 'expenses', 'mileage', 'documents', 'leave_balances', 'notifications']
            : ['documents', 'reports', 'expenses', 'mileage', 'hr', 'tasks', 'notifications'];
        return limitedFooterRoutes(preferred);
    }

    function footerNavigationRoutes() {
        return state.footerNavigationKeys.length
            ? limitedFooterRoutes(state.footerNavigationKeys.map(footerRoute))
            : defaultFooterRoutes();
    }

    function footerParentRoute(current, footerRoutes = footerNavigationRoutes()) {
        if (footerRoutes.includes(current)) return current;
        if (!isSalariedUser() && footerRoutes.includes('hr') && managerHrEntries().some(([route]) => route === current)) return 'hr';
        const parents = {
            leave_balances: ['absences'], messages: ['contacts'], employee_documents: ['hr'],
            hr_pending: ['hr'], sales_revenue: ['hr'], employee_mobile_access: ['hr'],
            task_create: ['tasks'], absence_create: ['absences', 'hr'],
            expense_create: ['expenses'], expense_resume: ['expenses'], expense_item_create: ['expenses'],
            mileage_create: ['mileage'], mileage_submit: ['mileage'], mileage_settings: ['mileage'],
            payslip_latest: ['payslips'], report_create: ['reports'],
        };
        return (parents[current] || []).find((route) => footerRoutes.includes(route))
            || (footerRoutes.includes('more') ? 'more' : profileLandingRoute());
    }

    function unreadBadgeMarkup() {
        return '<span class="mobile-app-unread-badge" data-mobile-unread-badge hidden></span>';
    }

    function navigationGlyph(route) {
        const paths = {
            billing: 'M19 3H5v18l3-2 4 2 4-2 3 2V3Zm-2 14.3-1-.7-4 2-4-2-1 .7V5h10v12.3ZM8 7h8v2H8V7Zm0 4h8v2H8v-2Z',
            home: 'M12 3 2 12h3v9h6v-6h2v6h6v-9h3L12 3Zm0 2.7 7 6.3v7h-4v-6H9v6H5v-7l7-6.3Z',
            documents: 'M20 6h-8l-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2Zm0 12H4V8h16v10Z',
            more: 'M6 10a2 2 0 1 1 0 4 2 2 0 0 1 0-4Zm6 0a2 2 0 1 1 0 4 2 2 0 0 1 0-4Zm6 0a2 2 0 1 1 0 4 2 2 0 0 1 0-4Z',
        };
        if (paths[route]) return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${paths[route]}"/></svg>`;
        return ({ reports: quickActionIcons.reports, payslips: quickActionIcons.last_payslip,
            leave_balances: quickActionIcons.absence, absences: quickActionIcons.absence,
            expenses: quickActionIcons.expense, mileage: quickActionIcons.mileage, hr: quickActionIcons.hr,
            tasks: quickActionIcons.task, notes: quickActionIcons.notes, notifications: quickActionIcons.notifications,
            contacts: quickActionIcons.contact, support: quickActionIcons.support })[route] || quickActionIcons.documents;
    }

    function navigationBar(current, { previewRoutes = null } = {}) {
        if (!state.activeDossierId && !previewRoutes) return '';
        const catalog = mobileNavigationCatalog();
        const footerRoutes = previewRoutes || footerNavigationRoutes();
        const selectedRoute = footerParentRoute(current, footerRoutes);
        const tabs = footerRoutes.map((route) => catalog.find(([candidate]) => candidate === route)).filter(Boolean);
        return `<nav class="mobile-app-nav${previewRoutes ? ' is-preview' : ''}" aria-label="${previewRoutes ? 'Aperçu de la navigation' : 'Navigation principale'}" style="--mobile-main-navigation-count:${tabs.length}">${tabs.map(([route, label, icon]) => {
            const badge = !previewRoutes && mobileNotificationsAllowed() && (route === 'notifications' || (route === 'more' && !footerRoutes.includes('notifications')));
            return `<button type="button" ${previewRoutes ? 'disabled tabindex="-1"' : ''} data-mobile-route="${route}" data-icon="${icon}" data-navigation-icon="${route}" aria-label="${escapeHtml(label)}" ${selectedRoute === route ? 'aria-current="page"' : ''} ${badge ? `data-mobile-badge-label="${escapeHtml(label)}"` : ''}><span class="mobile-app-nav__label" data-navigation-full-label="${escapeHtml(label)}">${escapeHtml(label)}</span>${badge ? unreadBadgeMarkup() : ''}</button>`;
        }).join('')}</nav>`;
    }

    function syncNotificationBadges() {
        const current = state.notificationBadgeScope === currentUploadIdentityScope();
        const count = current && Number.isInteger(state.notificationUnreadCount) ? state.notificationUnreadCount : null;
        const unavailable = current && state.notificationBadgeStatus === 'error';
        const suffix = count > 0 ? `${count} ${count === 1 ? 'notification non lue' : 'notifications non lues'}`
            : unavailable ? 'Nombre de notifications indisponible' : '';
        app.querySelectorAll('[data-mobile-unread-badge]').forEach((badge) => {
            badge.hidden = !(count > 0 || unavailable);
            badge.textContent = unavailable ? '?' : count > 99 ? '99+' : String(count || '');
            badge.classList.toggle('is-unknown', unavailable);
            badge.setAttribute('aria-hidden', 'true');
            const control = badge.closest('[data-mobile-badge-label]');
            if (control) control.setAttribute('aria-label', `${control.dataset.mobileBadgeLabel}${suffix ? ` - ${suffix}` : ''}`);
        });
    }

    async function refreshNotificationBadge(force = false) {
        if (!state.authenticated || !state.bootstrap || !mobileNotificationsAllowed() || document.hidden || state.applicationBackgrounded) return;
        const scope = currentUploadIdentityScope();
        if (!force && scope === state.notificationBadgeScope && (state.notificationBadgeOperation || Date.now() - state.notificationBadgeCheckedAt < 60000)) return;
        const operation = {};
        state.notificationBadgeOperation = operation;
        state.notificationBadgeScope = scope;
        state.notificationBadgeStatus = 'loading';
        state.notificationUnreadCount = null;
        state.notificationBadgeCheckedAt = Date.now();
        syncNotificationBadges();
        try {
            const response = await api('/notifications?unread=1&per_page=1', { expectedIdentityScope: scope });
            if (scope !== currentUploadIdentityScope() || state.notificationBadgeOperation !== operation) return;
            const total = response.meta?.pagination?.total;
            if (!Number.isInteger(total) || total < 0) throw new Error('Compteur indisponible.');
            state.notificationUnreadCount = total;
            state.notificationBadgeStatus = 'loaded';
        } catch (_) {
            if (scope !== currentUploadIdentityScope() || state.notificationBadgeOperation !== operation) return;
            state.notificationUnreadCount = null;
            state.notificationBadgeStatus = 'error';
        } finally {
            if (state.notificationBadgeOperation === operation) {
                state.notificationBadgeOperation = null;
                syncNotificationBadges();
            }
        }
    }
    ...
    function syncDocumentFolderSelectorScope(scope, folderId) {
        const allFolders = ['all', 'recent'].includes(scope);
        const toggle = app.querySelector('[data-mobile-document-folder-menu-toggle]');
        if (!toggle) return;
        const label = scope === 'recent' ? 'Récents' : allFolders
            ? 'Tous les dossiers'
    ...
    function renderFrame(title, content, options = {}) {
        ...
        const activeTab = footerParentRoute(state.route);
        ...
        const cameleonReturnControl = cameleonActive
            ? '<button type="button" role="menuitem" data-cameleon-stop><svg class="mobile-app-user-menu__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 7h9l-2.5-2.5L16 3l5 5-5 5-1.5-1.5L17 9H8V7Zm8 10H7l2.5 2.5L8 21l-5-5 5-5 1.5 1.5L7 15h9v2Z"/></svg><span>Revenir à mon compte</span></button>'
            : '';
        const cameleonPickerControl = hasCameleonPicker
            ? `<button type="button" role="menuitem" data-mobile-cameleon-menu-toggle aria-expanded="false"><svg class="mobile-app-user-menu__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 7h9l-2.5-2.5L16 3l5 5-5 5-1.5-1.5L17 9H8V7Zm8 10H7l2.5 2.5L8 21l-5-5 5-5 1.5 1.5L7 15h9v2Z"/></svg><span>Voir l’app en tant que…</span><svg class="mobile-app-user-menu__chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="m7 9.5 5 5 5-5 1.4 1.4-6.4 6.4-6.4-6.4L7 9.5Z"/></svg></button><div class="mobile-app-cameleon-menu" data-mobile-cameleon-menu hidden><input type="search" placeholder="Rechercher une personne" aria-label="Rechercher une personne" autocomplete="off" data-mobile-cameleon-search><div class="mobile-app-cameleon-menu__results" data-mobile-cameleon-results><p>Chargement…</p></div></div>`
            : '';
        ...
        const menuIdentity = `<div class="mobile-app-user-menu__identity"><strong>${escapeHtml(headerSubtitle)}</strong><span>${escapeHtml(headerRole)}</span>${cameleonActive ? '<b class="mobile-app-user-menu__mode">Mode Caméléon</b>' : ''}</div>`;
        const menuNotifications = mobileNotificationsAllowed()
            ? `<button type="button" role="menuitem" data-mobile-route="notifications" data-mobile-badge-label="Notifications"><span class="mobile-app-user-menu__icon">${quickActionIcons.notifications}</span><span>Notifications</span>${unreadBadgeMarkup()}</button>` : '';
        const menuAbout = `<button type="button" role="menuitem" data-mobile-route="about"><svg class="mobile-app-user-menu__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 2a8 8 0 1 1 0 16 8 8 0 0 1 0-16Zm-1 6h2v7h-2V10Zm0-3h2v2h-2V7Z"/></svg><span>À propos et mises à jour</span></button>`;
        const identityControlWithTheme = headerSubtitle ? `<div class="mobile-app-topbar__identity"><button class="mobile-app-topbar__user${cameleonActive ? ' is-cameleon' : ''}" type="button" data-mobile-user-menu-toggle aria-label="Compte - ${escapeHtml(headerSubtitle)} - ${escapeHtml(headerRole)}${cameleonActive ? ' - Mode Caméléon' : ''}" aria-haspopup="menu" aria-expanded="false"><span class="mobile-app-topbar__user-copy"><span class="mobile-app-topbar__user-name" data-mobile-header-user-name data-full-name="${escapeHtml(headerSubtitle)}">${escapeHtml(headerSubtitle)}</span><span class="mobile-app-topbar__user-role">${escapeHtml(headerRole)}${cameleonActive ? ' - Caméléon' : ''}</span></span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 9.5 5 5 5-5 1.4 1.4-6.4 6.4-6.4-6.4L7 9.5Z"/></svg></button><div class="mobile-app-user-menu has-theme-quick" role="menu" aria-label="Compte" data-mobile-user-menu hidden>${menuIdentity}<button type="button" role="menuitem" data-mobile-route="user_settings"><svg class="mobile-app-user-menu__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M19.4 13a7.8 7.8 0 0 0 0-2l2.1-1.6-2-3.5-2.5 1a7.6 7.6 0 0 0-1.7-1L15 3.2h-4L10.6 6a7.6 7.6 0 0 0-1.7 1l-2.5-1-2 3.5L6.5 11a7.8 7.8 0 0 0 0 2l-2.1 1.6 2 3.5 2.5-1a7.6 7.6 0 0 0 1.7 1l.4 2.8h4l.4-2.8a7.6 7.6 0 0 0 1.7-1l2.5 1 2-3.5-2.2-1.6ZM13 19h-1l-.3-2.4-.7-.3a5.6 5.6 0 0 1-1.3-.8l-.6-.5-2.2.9-.5-.9 1.9-1.4-.1-.8a5.7 5.7 0 0 1 0-1.6l.1-.8L6.4 9l.5-.9 2.2.9.6-.5a5.6 5.6 0 0 1 1.3-.8l.7-.3L12 5h1l.3 2.4.7.3a5.6 5.6 0 0 1 1.3.8l.6.5 2.2-.9.5.9-1.9 1.4.1.8a5.7 5.7 0 0 1 0 1.6l-.1.8 1.9 1.4-.5.9-2.2-.9-.6.5a5.6 5.6 0 0 1-1.3.8l-.7.3L13 19Zm-.5-3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm0-2a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3Z"/></svg><span>Paramètres utilisateur</span></button>${menuNotifications}${menuAbout}${quickThemeControl}${cameleonMenuControl}<button class="mobile-app-user-menu__logout" type="button" role="menuitem" data-mobile-logout><svg class="mobile-app-user-menu__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M10 4H5v16h5v-2H7V6h3V4Zm5.6 3.4L14.2 8.8l2.2 2.2H9v2h7.4l-2.2 2.2 1.4 1.4 4.6-4.6-4.6-4.6Z"/></svg><span>Se déconnecter</span></button></div></div>` : '';
        const mainHrSection = !isSalariedUser() && options.tab === 'hr' && managerHrEntries().some(([route]) => route === state.route);
        const showBack = state.route !== 'home' && !mainHrSection && !footerNavigationRoutes().includes(state.route);
        const headerBack = showBack ? '<button class="mobile-app-topbar__back" type="button" data-mobile-header-back aria-label="Revenir à l’écran précédent"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 4-8 8 8 8 1.4-1.4L7.8 13H21v-2H7.8l5.6-5.6L12 4Z"/></svg></button>' : '';
        const hasHeaderContext = Boolean(assistantControl || contextControl);
        app.innerHTML = `<div class="mobile-app-shell ${bottomPanel ? 'has-bottom-panel' : ''}">
            <div>${banners}<header class="mobile-app-topbar ${hasHeaderContext ? 'has-header-context' : ''}"><div class="mobile-app-topbar__heading">${identityControlWithTheme}</div>
                ...
                ${headerTitle ? `<div class="mobile-app-topbar__page-title">${headerBack}<h1 class="mobile-app-topbar__title">${escapeHtml(headerTitle)}</h1></div>` : ''}
            ...
            <main class="mobile-app-content ${escapeHtml(contentClass)}">${hrSelector}${managerHrEntries().some(([key]) => key === state.route) ? hrOverviewMarkup() : ''}${content}</main>
    ...
    function closeHeaderMenus({ focus = false } = {}) {
        const userOpen = app.querySelector('[data-mobile-user-menu]:not([hidden])');
        const dossierOpen = app.querySelector('[data-mobile-dossier-menu]:not([data-mobile-document-folder-menu]):not([hidden])');
        app.querySelectorAll('[data-mobile-user-menu], [data-mobile-cameleon-menu], [data-mobile-dossier-menu]:not([data-mobile-document-folder-menu])').forEach((menu) => { menu.hidden = true; });
        app.querySelectorAll('[data-mobile-user-menu-toggle], [data-mobile-cameleon-menu-toggle], [data-mobile-dossier-menu-toggle]').forEach((button) => button.setAttribute('aria-expanded', 'false'));
        state.cameleonRequestSequence += 1;
        if (focus) app.querySelector(userOpen ? '[data-mobile-user-menu-toggle]' : dossierOpen ? '[data-mobile-dossier-menu-toggle]' : '[data-mobile-user-menu-toggle]')?.focus({ preventScroll: true });
    }

    function fitHeaderMenus() {
        ...
        app.querySelectorAll('[data-mobile-user-menu]:not([hidden]), [data-mobile-dossier-menu]:not([data-mobile-document-folder-menu]):not([hidden])').forEach((menu) => {
            menu.style.maxHeight = `${Math.max(48, Math.min(bounds.bottom, viewportBottom) - menu.getBoundingClientRect().top - 8)}px`;
            menu.style.maxWidth = `${Math.max(160, bounds.width - 32)}px`;
        });
    }

    const mobileViewportHeights = {};
    function syncMobileKeyboardLayout() {
        const orientation = iosRuntime && window.screen.orientation?.type
            ? window.screen.orientation.type.split('-')[0] : window.innerWidth > window.innerHeight ? 'landscape' : 'portrait';
        const largeNavigationText = Number.parseFloat(getComputedStyle(app).getPropertyValue('--mobile-home-font-scale')) >= 1.3;
        app.querySelector('.mobile-app-shell')?.classList.toggle('has-large-navigation-text', largeNavigationText);
        app.querySelectorAll('[data-navigation-full-label]').forEach((label) => {
            label.textContent = largeNavigationText && label.dataset.navigationFullLabel === 'Documents' ? 'Docs' : label.dataset.navigationFullLabel;
        });
        const height = window.visualViewport?.height || window.innerHeight;
        const focused = app.contains(document.activeElement) && document.activeElement?.matches('input:not([type="checkbox"]):not([type="radio"]):not([type="button"]), textarea, [contenteditable="true"]');
        if (!focused) mobileViewportHeights[orientation] = Math.max(mobileViewportHeights[orientation] || 0, height);
        const baseline = Math.max(mobileViewportHeights[orientation] || height, window.innerHeight);
        app.querySelector('.mobile-app-shell')?.classList.toggle('is-keyboard-visible', iosRuntime && focused && baseline - height > 100);
        fitHeaderMenus();
    }

    function mobileInstantLabel(value) {
        if (!value) return 'Pas encore vérifiée';
        const shared = window.ComptaDateTime;
        if (shared?.format && (!state.accountTimezone || shared.timezone?.() === state.accountTimezone)) return shared.format(value, 'd/m/Y H:i');
        // The shared site formatter uses the owner's body timezone; a Caméléon actor may differ.
        try { return new Intl.DateTimeFormat('fr-FR', { dateStyle: 'short', timeStyle: 'short', ...(state.accountTimezone ? { timeZone: state.accountTimezone } : {}) }).format(new Date(value)); }
        catch (_) { return 'Date indisponible'; }
    }

    function renderAbout() {
        const checked = Boolean(state.updateCheckedAtUtc);
        const update = checked && ['optional', 'mandatory'].includes(String(state.config?.update_mode));
        const latest = Number(state.config?.latest_version_code);
        const knownCurrent = checked && Number.isFinite(latest) && latest > 0 && latest <= state.versionCode;
        const message = state.updateChecking ? 'Vérification en cours…'
            : state.updateCheckError ? state.updateCheckError
                : update ? `Version ${String(state.config?.latest_version_name || '')} disponible.`
                    : knownCurrent ? 'Votre version est à jour pour ce canal.'
                        : checked && latest === 0 ? 'Aucune version publiée pour ce canal.' : 'Recherchez les mises à jour disponibles.';
        const channel = String(state.config?.channel || nativeAppMetadata?.channel || 'beta');
        const channelLabel = channel === 'beta' ? 'Bêta Maison Pilote' : ['production', 'stable'].includes(channel) ? 'Version stable' : 'Canal de l’application';
        renderFrame('À propos et mises à jour', `<div class="mobile-app-about">
            <section class="mobile-app-form-panel"><strong>Maison Pilote</strong><dl><div><dt>Version installée</dt><dd>${escapeHtml(state.versionName)} (${state.versionCode})</dd></div><div><dt>Canal</dt><dd>${escapeHtml(channelLabel)}</dd></div><div><dt>Dernière vérification</dt><dd>${state.updateCheckedAtUtc ? `<time datetime="${escapeHtml(state.updateCheckedAtUtc)}">${escapeHtml(mobileInstantLabel(state.updateCheckedAtUtc))}</time>` : 'Pas encore vérifiée'}</dd></div></dl></section>
            <section class="mobile-app-form-panel"><p role="status" class="${state.updateCheckError ? 'mobile-app-settings-message is-error' : ''}">${escapeHtml(message)}</p><button class="mobile-app-primary-button" type="button" data-mobile-update-check ${state.updateChecking ? 'disabled' : ''}>${state.updateChecking ? 'Vérification…' : 'Rechercher une mise à jour'}</button>${update && !state.updateCheckError ? '<button class="mobile-app-secondary-button" type="button" data-mobile-update>Mettre à jour</button>' : ''}</section>${mobilePrivacyLink()}</div>`, { tab: 'more' });
    }

    async function checkForUpdates(manual = false) {
        if (!state.authenticated || state.updateChecking || document.hidden || state.applicationBackgrounded) return;
        if (!manual && state.updateCheckedAtUtc && Date.now() - Date.parse(state.updateCheckedAtUtc) < 60000) return;
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const operation = {};
        state.updateCheckOperation = operation;
        state.updateChecking = true;
        state.updateCheckError = '';
        if (state.route === 'about') renderAbout();
        try {
            const [response, settings] = await Promise.all([
                api('/config', { expectedIdentityScope: scope, offlineCache: false }),
                api('/user-settings', { expectedIdentityScope: scope }).catch(() => null),
            ]);
            if (scope !== currentUploadIdentityScope() || state.updateCheckOperation !== operation) return;
            if (!response.data || !Number.isFinite(Number(response.data.latest_version_code))) throw new Error('La réponse de vérification est incomplète.');
            state.config = response.data;
            state.accountTimezone = String(settings?.data?.timezone || state.accountTimezone || '');
            const checkedAt = Date.parse(String(response.meta?.server_time_utc || ''));
            state.updateCheckedAtUtc = Number.isFinite(checkedAt) ? new Date(checkedAt).toISOString() : new Date().toISOString();
        } catch (error) {
            if (scope !== currentUploadIdentityScope() || state.updateCheckOperation !== operation) return;
            state.updateCheckError = `Vérification impossible. ${String(error?.message || 'Réessayez lorsque la connexion est disponible.')}`;
        } finally {
            if (state.updateCheckOperation === operation) {
                state.updateChecking = false;
                state.updateCheckOperation = null;
                if (scope === currentUploadIdentityScope() && state.route === 'about') renderAbout();
            }
        }
    }
    ...
    function normalizedNotificationRouteParams(raw = {}) {
        ...
        if (['receivables', 'quotes', 'overdue'].includes(raw.view)) params.view = raw.view;
        return params;
    }
    ...
    function notificationDestinationRoute(destination) {
        ...
        if (value.startsWith('billing')) return 'billing';
    ...
    function renderLoading(title = 'Chargement') {
        const showNews = state.route === 'home' && state.authenticated && state.bootstrap;
        renderFrame(title, `<div class="mobile-app-loading"><span>Chargement…</span></div>${showNews ? homeNewsMarkup() : ''}`);
        if (showNews) startHomeNewsRefresh();
    }
    ...
    function renderError(error, retryRoute = state.route) {
        ...
        const retryParams = retryRoute === state.route ? state.routeParams : {};
        const showNews = state.route === 'home' && retryRoute === 'home' && state.authenticated && state.bootstrap;
        renderFrame(title, `<div class="mobile-app-error"><strong>${escapeHtml(message)}</strong><p>${escapeHtml(guidance)}</p>${requestId}<div class="mt-3"><button class="mobile-app-secondary-button" type="button" data-mobile-route="${escapeHtml(retryRoute)}" data-mobile-retry-params="${escapeHtml(JSON.stringify(retryParams))}">Réessayer</button></div></div>${showNews ? homeNewsMarkup() : ''}`);
        if (showNews) startHomeNewsRefresh();
    }
    ...
    async function restoreIosPwaSession() {
        ...
        try {
            ...
            clearMobileAccessSession({ preserveNavigation: true });
            await purgeOfflineCache();
            return false;
        }
    }
    ...
    async function loadBootstrap(sequence = null) {
        const previousScope = currentUploadIdentityScope();
        const hadProfile = Boolean(state.bootstrap?.profile);
        ...
        if (previousScope !== currentUploadIdentityScope()) {
            state.footerNavigationOperation = null;
            state.footerNavigationSaving = false;
            state.footerNavigationEditable = false;
            state.footerNavigationDraftScope = '';
            state.notificationBadgeOperation = null;
            state.notificationUnreadCount = null;
            state.notificationBadgeStatus = 'idle';
            state.updateCheckOperation = null;
            state.updateChecking = false;
            state.accountTimezone = '';
            if (hadProfile) { state.routeHistory = []; state.routeScrollPositions = {}; state.mainTabParams = {}; }
        }
        void refreshNotificationBadge(true);
        const ownerId = Number(response.data.profile?.owner_user_id || response.data.profile?.id) || 0;
        if (ownerId) window.MaisonPiloteNative?.navigation?.bindIdentity?.(ownerId);
        return true;
    }
    ...
    function beginApplicationBackground() {
        ...
        if (iosRuntime && nativeBiometricBridgeAvailable && state.biometricEnabled
            && mobileFactorSettings?.panel?.isConnected && !state.cameleonSessionId) {
            // Keep an in-memory draft while an authenticator is open; biometric
            // relocking may replace the surrounding page on return.
            mobileFactorSettings.suspendedAt = Date.now();
            mobileFactorSettings.panel.remove();
            clearTimeout(mobileFactorExpiryTimer);
            mobileFactorExpiryTimer = setTimeout(forgetMobileFactorSettings, 300000);
        }
    ...
    async function navigate(route, params = {}, remember = true) {
        const departingScroll = app.querySelector('.mobile-app-content')?.scrollTop || 0;
        const departingKey = `${currentUploadIdentityScope()}:${state.route}:${JSON.stringify(state.routeParams)}`;
        state.routeScrollPositions[departingKey] = departingScroll;
        if (footerNavigationRoutes().includes(state.route)) state.mainTabParams[state.route] = { ...state.routeParams };
        state.offlineCacheActive = false;
        state.offlineCacheSavedAtUtc = null;
        ...
        if (state.route === 'notes' && !await flushMobileNotes()) return false;
        stopHomeNewsRefresh();
        stopHomePreferenceRequests();
        ...
        stopMessagePolling();
        if (route !== 'messages') state.messageRequestSequence += 1;
        stopLeaveBalancePolling();
        ...
        if (state.bootstrap && ['expenses', 'expense_create', 'expense_resume', 'expense_item_create'].includes(route)
            && !(can('expenses.manage_own') || can('expenses.create') || can('expenses.approve'))) {
            route = profileLandingRoute();
            params = {};
            state.route = route;
            state.routeParams = {};
            state.routeHistory = state.routeHistory.filter((entry) => !['expenses', 'expense_create', 'expense_resume', 'expense_item_create'].includes(entry.route));
        }
        ...
        const destinationAllowed = assistantRequested || route === requestedRoute;
        ...
        try {
            if (grhRoutes.has(route) && (can('absences.approve') || can('expenses.approve'))) {
                try {
                    ...
                    state.hrPendingValidationCount = null;
                    state.hrPendingValidations = null;
                    state.hrPendingError = { message: 'Les demandes à valider n’ont pas pu être chargées.' };
            ...
            switch (route) {
                ...
                case 'footer_settings': await loadFooterNavigationSettings(); break;
                case 'support':
                ...
                case 'expense_resume': await renderExpenseResume(params); break;
                ...
                case 'billing': await renderBilling(params); break;
                ...
                case 'about': renderAbout(); void checkForUpdates(true); break;
                case 'desktop': renderDesktopLink(); break;
                default: await renderHome();
            }
            if (navigationGeneration === state.navigationGeneration && state.route === route) {
                const key = `${currentUploadIdentityScope()}:${route}:${JSON.stringify(params)}`;
                const content = app.querySelector('.mobile-app-content');
                if (content && Number.isFinite(state.routeScrollPositions[key])) content.scrollTop = state.routeScrollPositions[key];
            }
            if (assistantRequested && isAdminUser()) window.setTimeout(startAssistantRecognition, 150);
            return destinationAllowed && navigationGeneration === state.navigationGeneration && state.route === route;
        } catch (error) {
            if (navigationGeneration !== state.navigationGeneration || state.route !== route) return false;
            if (error.status === 426) {
    ...
    function renderMore() {
        ...
        const actions = mobileNavigationCatalog()
            ...
            .map(([route, label, , subtitle]) => [
                route,
                route === 'contacts' ? mobileReferentContactLabel() : route === 'notifications' ? 'Notifications' : label,
                route === 'contacts'
        ...
        const content = `<div class="mobile-app-more-list">${actions.map(([route, label, subtitle, icon]) => `
            <button class="mobile-app-more-action" type="button" data-mobile-route="${route}" ${route === 'notifications' ? 'data-mobile-badge-label="Notifications"' : ''}>
                <span class="mobile-app-more-action__icon">${icon}</span>
                <span class="mobile-app-more-action__copy"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(subtitle)}</span></span>${route === 'notifications' ? unreadBadgeMarkup() : ''}
            </button>
        `).join('')}</div>${footerSettings}`;
        renderFrame('Plus', content, { tab: 'more' });
    }

    function rememberFooterDraft() {
        const scope = currentUploadIdentityScope();
        state.footerNavigationDrafts[scope] = [...state.footerNavigationDraftKeys];
        try { sessionStorage.setItem(`${navigationSessionStorageKey}:footer-drafts`, JSON.stringify(Object.fromEntries(Object.entries(state.footerNavigationDrafts).slice(-20)))); } catch (_) { /* In-memory draft remains available. */ }
    }

    async function loadFooterNavigationSettings() {
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        if (state.footerNavigationSaving && state.footerNavigationDraftScope === scope) {
            renderFooterNavigationSettings();
            return;
        }
        let response;
        try {
            response = await api('/navigation-preferences/footer', { expectedIdentityScope: scope });
        } catch (error) {
            if (scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration || state.route !== 'footer_settings') return;
            throw error;
        }
        if (scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration || state.route !== 'footer_settings') return;
        state.footerNavigationKeys = Array.isArray(response.data?.footer_navigation_keys) ? response.data.footer_navigation_keys.map(String) : [];
        state.footerNavigationEditable = response.data?.editable === true;
        renderFooterNavigationSettings(true);
    }

    function renderFooterNavigationSettings(reset = false) {
        const previousScroll = !reset && app.querySelector('.mobile-app-footer-settings')
            && state.footerNavigationDraftScope === currentUploadIdentityScope()
            ? (app.querySelector('.mobile-app-content')?.scrollTop || 0) : 0;
        const catalog = mobileNavigationCatalog();
        const scope = currentUploadIdentityScope();
        if (reset || state.footerNavigationDraftScope !== scope || !state.footerNavigationDraftKeys.length) {
            try { state.footerNavigationDrafts = JSON.parse(sessionStorage.getItem(`${navigationSessionStorageKey}:footer-drafts`) || '{}') || {}; } catch (_) { state.footerNavigationDrafts = {}; }
            if (typeof state.footerNavigationDrafts !== 'object' || Array.isArray(state.footerNavigationDrafts)) state.footerNavigationDrafts = {};
            const draft = state.footerNavigationDrafts[scope];
            state.footerNavigationDraftKeys = Array.isArray(draft) ? limitedFooterRoutes(draft) : footerNavigationRoutes();
            state.footerNavigationDraftScope = scope;
            state.footerNavigationMessage = '';
        }
        const selected = state.footerNavigationDraftKeys.map((route) => catalog.find(([candidate]) => candidate === route)).filter(Boolean);
        const hidden = catalog.filter(([route]) => !state.footerNavigationDraftKeys.includes(route));
        const locked = state.readOnly || !state.footerNavigationEditable || state.footerNavigationSaving;
        const row = ([route, label], index, isSelected) => {
            const required = ['home', 'more'].includes(route);
            return `<article class="mobile-app-footer-setting-row${required ? ' is-required' : isSelected ? ' is-selected' : ' is-available'}">
                <span class="mobile-app-footer-setting-row__icon" data-route="${escapeHtml(route)}" aria-hidden="true">${navigationGlyph(route)}</span>
                <span class="mobile-app-footer-setting-row__copy"><strong>${escapeHtml(route === 'notifications' ? 'Notifications' : label)}</strong>${required ? '<small>Toujours affiché</small>' : ''}</span>
                ${isSelected && !required ? `<button type="button" data-mobile-footer-move="up" data-mobile-footer-route="${route}" aria-label="Monter ${escapeHtml(label)}" ${locked || index <= 1 ? 'disabled' : ''}>↑</button><button type="button" data-mobile-footer-move="down" data-mobile-footer-route="${route}" aria-label="Descendre ${escapeHtml(label)}" ${locked || index >= selected.length - 2 ? 'disabled' : ''}>↓</button>` : ''}
                ${!required ? `<button type="button" data-mobile-footer-toggle="${route}" aria-label="${isSelected ? 'Retirer' : 'Ajouter'} ${escapeHtml(label)}" ${locked || (!isSelected && selected.length >= 5) ? 'disabled' : ''}>${isSelected ? '−' : '+'}</button>` : ''}
            </article>`;
        };
        ...
        const content = `<div class="mobile-app-footer-settings">
            <div class="mobile-app-footer-settings__intro"><p>Pour votre compte</p><p>5 boutons maximum - ${selected.length} ${selected.length === 1 ? 'bouton affiché' : 'boutons affichés'}</p></div>
            ${navigationBar('more', { previewRoutes: state.footerNavigationDraftKeys })}
            ${!state.footerNavigationEditable ? '<p class="mobile-app-footer-settings__message" role="status">La navigation de cet aperçu ne peut pas être enregistrée.</p>' : ''}
            ...
            ${message}
            <button class="mobile-app-secondary-button" type="button" data-mobile-footer-reset ${locked ? 'disabled' : ''}>Rétablir par défaut</button>
            <button class="mobile-app-primary-button" type="button" data-mobile-footer-save ${locked ? 'disabled' : ''}>${state.footerNavigationSaving ? 'Enregistrement…' : 'Enregistrer'}</button>
        </div>`;
        renderFrame('Personnaliser la navigation', content, { tab: 'more' });
        const viewport = app.querySelector('.mobile-app-content');
        if (viewport) {
            viewport.scrollTop = previousScroll;
            const feedback = viewport.querySelector('.mobile-app-footer-settings__message');
            if (feedback) {
                const bounds = viewport.getBoundingClientRect();
                const result = feedback.getBoundingClientRect();
                if (result.bottom > bounds.bottom - 12) viewport.scrollTop += result.bottom - bounds.bottom + 12;
                else if (result.top < bounds.top + 12) viewport.scrollTop += result.top - bounds.top - 12;
            }
        }
    }

    async function saveFooterNavigation() {
        if (state.readOnly || !state.footerNavigationEditable || state.footerNavigationSaving) return;
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const operation = {};
        const sent = [...state.footerNavigationDraftKeys];
        const visible = mobileNavigationCatalog().map(([route]) => footerStoredKey(route));
        state.footerNavigationOperation = operation;
        state.footerNavigationSaving = true;
        state.footerNavigationMessage = '';
        renderFooterNavigationSettings();
        try {
            const response = await api('/navigation-preferences/footer', {
                method: 'PATCH', expectedIdentityScope: scope,
                body: { footer_navigation_keys: sent.map(footerStoredKey), visible_navigation_keys: visible },
            });
            if (scope !== currentUploadIdentityScope() || state.footerNavigationOperation !== operation) return;
            if (!Array.isArray(response.data?.footer_navigation_keys)) throw new Error('La confirmation du serveur est incomplète.');
            state.footerNavigationKeys = response.data.footer_navigation_keys.map(String);
            delete state.footerNavigationDrafts[scope];
            try { sessionStorage.setItem(`${navigationSessionStorageKey}:footer-drafts`, JSON.stringify(state.footerNavigationDrafts)); } catch (_) { /* Confirmed preference is on the server. */ }
            state.footerNavigationDraftKeys = footerNavigationRoutes();
            state.footerNavigationMessage = 'Navigation enregistrée.';
        } catch (error) {
            if (scope !== currentUploadIdentityScope() || state.footerNavigationOperation !== operation) return;
            state.footerNavigationMessage = `Impossible d’enregistrer la navigation. ${String(error?.message || '')}`.trim();
        } finally {
            if (state.footerNavigationOperation === operation) {
                state.footerNavigationOperation = null;
                state.footerNavigationSaving = false;
                if (scope === currentUploadIdentityScope() && state.route === 'footer_settings') renderFooterNavigationSettings();
            }
        }
    }

    function mobileDataScope() {
        return currentUploadIdentityScope();
    }

    function paginationMarkup(key) {
        const list = state.paginatedLists[key];
        return `<div data-mobile-paginated-list="${key}">${list.render(list.items)}${list.pagination?.has_more ? `<button class="mobile-app-secondary-button mobile-app-load-more" type="button" data-mobile-load-more="${key}">Charger la suite</button>` : ''}</div>`;
    }

    function preparePaginatedList(key, url, response, items, render) {
        state.paginatedLists[key] = { url, items, render, pagination: response.meta?.pagination, scope: mobileDataScope(), route: state.route, busy: false };
        return paginationMarkup(key);
    }

    async function loadMoreMobileItems(key, button) {
        const list = state.paginatedLists[key];
        if (!list || list.busy || !list.pagination?.has_more) return;
        list.busy = true;
        button.disabled = true;
        button.textContent = 'Chargement…';
        try {
            const url = new URL(list.url, 'https://mobile.invalid');
            url.searchParams.set('page', String(Number(list.pagination.current_page) + 1));
            const response = await api(`${url.pathname}${url.search}`, key === 'notifications' ? { expectedIdentityScope: list.scope } : {});
            if (state.paginatedLists[key] !== list || list.scope !== mobileDataScope() || state.route !== (list.route || key)) return;
            const extra = key === 'support' ? response.data?.tickets : response.data;
            const seen = new Set(list.items.map((item) => item.id));
            list.items = [...list.items, ...(Array.isArray(extra) ? extra : []).filter((item) => !seen.has(item.id))];
            list.pagination = response.meta?.pagination;
            const node = app.querySelector(`[data-mobile-paginated-list="${key}"]`);
            const viewport = key === 'notifications' ? app.querySelector('.mobile-app-content') : null;
            const top = viewport?.getBoundingClientRect().top;
            const anchor = viewport && [...node?.querySelectorAll('[data-notification-visible]') || []]
                .find((card) => card.getBoundingClientRect().bottom > top);
            const anchorId = anchor?.dataset.notificationVisible;
            const anchorTop = anchor?.getBoundingClientRect().top;
            if (node) node.outerHTML = paginationMarkup(key);
            if (key === 'documents') { syncDocumentSelectionUi(); enhanceDocumentFolderFlow(); }
            if (key === 'reports') scheduleMobileReportRefresh(list.items);
            if (key === 'notifications') {
                state.notificationItems = list.items;
                const replacement = anchorId && app.querySelector(`[data-notification-visible="${anchorId}"]`);
                if (viewport && replacement) viewport.scrollTop += replacement.getBoundingClientRect().top - anchorTop;
                startNotificationVisibilityTracking();
            }
        } catch (error) {
            showQuickCreationNotice(error?.message || 'Chargement impossible. Réessayez.');
        } finally {
            list.busy = false;
            if (button.isConnected) { button.disabled = false; button.textContent = 'Charger la suite'; }
        }
    }
    ...
    async function renderSupport(message = '') {
        ...
        const query = new URLSearchParams({ statuses: statuses.join(','), per_page: '20' });
        ...
        const renderTickets = (tickets) => tickets.length ? `<div class="mobile-app-support-list"><p class="mobile-app-support-count">${Number(data.total) || tickets.length} ${Number(data.total || tickets.length) === 1 ? 'ticket' : 'tickets'}</p>${tickets.map((ticket) => {
        ...
        const list = preparePaginatedList('support', `/support/tickets?${query}`, response, tickets, renderTickets);
    ...
    async function renderHome() {
        ...
        if (!state.activeDossierId) {
            state.homeQuickActionScope = '';
            state.homeQuickActionsEditable = false;
        ...
        if (requestSequence !== state.homeRequestSequence || requestedIdentity !== currentIdentity || state.route !== 'home') return;
        ...
        state.homeQuickActionDefaultKeys = (response.data.quick_action_default_keys || []).map(String)
            .filter((key) => state.homeQuickActionCatalog.some((action) => String(action.key) === key));
        state.homeAttentionItems = (response.data.attention_items || []).filter((item) => Number(item.count) > 0).slice(0, 3);
        state.homeResumeAction = response.data.resume_action || null;
        state.homeQuickActionScope = mobileDraftScope();
        restoreSecureHomeDraft();
        const draft = state.homeQuickActionsEditable ? state.homeQuickActionDrafts[state.homeQuickActionScope] : null;
        if (draft) state.homeQuickActionKeys = draft.keys.filter((key) => state.homeQuickActionCatalog.some((action) => String(action.key) === key));
        state.quickActionsEditing = Boolean(draft);
        state.quickActionsDialogOpen = false;
        state.quickActionsSaving = false;
        state.quickActionsError = '';
    ...
    function renderHomeWelcome() {
        renderFrame('Accueil', `<section class="mobile-app-home-welcome"><h2>Votre compte est connecté</h2><p>Aucun dossier ne vous est encore attribué. Vous pouvez actualiser après votre rattachement.</p><div class="mobile-app-home-welcome__actions"><button type="button" class="mobile-app-primary-button" data-mobile-home-refresh>Actualiser</button><button type="button" class="mobile-app-secondary-button" data-mobile-route="user_settings">Mon compte</button><button type="button" class="mobile-app-secondary-button" data-mobile-logout>Se déconnecter</button></div></section><div class="mobile-app-home-personalize" data-mobile-home-personalize>${homePersonalizeButton()}</div>${homeNewsMarkup()}`, { tab: 'home' });
        startHomeNewsRefresh();
    }

    function canPersonalizeHome() {
        return state.quickActionsEditing || state.homeQuickActionsEditable || (!state.readOnly && state.homePreferences?.editable !== false);
    }

    function homePersonalizeButton() {
        if (!canPersonalizeHome()) return '';
        return `<button class="mobile-app-quick-actions-edit" type="button" data-quick-actions-edit ${state.quickActionsSaving || state.homePreferencesSaving ? 'disabled' : ''}>${state.quickActionsSaving ? 'Enregistrement…' : (state.quickActionsEditing ? 'Terminer' : 'Personnaliser')}</button>`;
    }

    function rerenderPersonalizedHome() {
        if (state.activeDossierId) renderQuickActionsHome();
    ...
    function resetMobileNotesState({ preserveActiveTab = false } = {}) {
        ...
        state.notesGeneration += 1;
        state.notesSavePromises = {};
    ...
    async function renderNotes() {
        const scope = mobileDraftScope();
        if (state.notesScope !== scope) {
        ...
        if (generation !== state.notesGeneration || scope !== mobileDraftScope() || state.route !== 'notes') return;
        state.notesPayload = response.data || {};
        restoreSecureNoteDrafts();
    ...
    async function saveMobileNote(scope) {
        ...
        if (state.notesSavePromises[scope]) {
            const succeeded = await state.notesSavePromises[scope];
            return succeeded ? saveMobileNote(scope) : false;
        }
        if (!Object.prototype.hasOwnProperty.call(state.notesDrafts, scope)) return true;
        if (state.notesScope !== mobileDraftScope()) return false;
        const tab = mobileNotesTabs()[scope];
        if (!mobileNotesWritable(tab)) return false;
        const submittedBody = String(state.notesDrafts[scope]);
        if (submittedBody === String(tab?.body || '')) {
            delete state.notesDrafts[scope];
            writeSecureMobileDraft(`note:${mobileDraftScope()}:${scope}`, null);
            return true;
        }
        const generation = state.notesGeneration;
        const draftKey = `note:${mobileDraftScope()}:${scope}`;
        const path = mobileNotesPath(`/${encodeURIComponent(scope)}`);
        state.notesSavingCount += 1;
        setMobileNotesStatus();
        const pending = (async () => {
            try {
                const response = await api(path, { method: 'PATCH', body: { body: submittedBody } });
                if (generation !== state.notesGeneration) return false;
                if (response.data?.tab) mobileNotesTabs()[scope] = response.data.tab;
                else mobileNotesTabs()[scope] = { ...tab, body: submittedBody };
                if (String(state.notesDrafts[scope]) === submittedBody) { delete state.notesDrafts[scope]; writeSecureMobileDraft(draftKey, null); }
                setMobileNotesStatus('À jour');
                return true;
            } catch (error) {
                if (generation === state.notesGeneration) setMobileNotesStatus(error?.message || 'Sauvegarde impossible.');
                return false;
            } finally {
                if (generation === state.notesGeneration) {
                    delete state.notesSavePromises[scope];
                    state.notesSavingCount = Math.max(0, state.notesSavingCount - 1);
                    setMobileNotesStatus(state.notesMessage);
                }
            }
        })();
        state.notesSavePromises[scope] = pending;
        const succeeded = await pending;
        return succeeded && generation === state.notesGeneration ? saveMobileNote(scope) : false;
    }

    async function flushMobileNotes() {
        if (state.notesScope && state.notesScope !== mobileDraftScope()) {
            resetMobileNotesState({ preserveActiveTab: true });
            return true;
        }
        const results = await Promise.all(Object.keys(state.notesDrafts).map((scope) => saveMobileNote(scope)));
    ...
    function quickActionMarkup(action) {
        if (state.quickActionsEditing && state.homeQuickActionsEditable) {
    ...
    function homeNewsDate(value, withTime = false) {
        if (!value) return '';
        const date = new Date(value);
        if (Number.isNaN(date.getTime())) return '';
        const timezone = String(state.homeNews?.timezone || '');
        const options = { day: '2-digit', month: '2-digit', year: 'numeric', ...(timezone ? { timeZone: timezone } : {}) };
        if (!timezone || window.ComptaDateTime?.timezone?.() === timezone) {
            const formatted = window.ComptaDateTime?.format?.(value, withTime ? 'd/m/Y à H:i' : 'd/m/Y', '');
            if (formatted) return formatted;
        }
        try {
            const day = new Intl.DateTimeFormat('fr-FR', options).format(date);
            return withTime ? `${day} à ${new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit', ...(timezone ? { timeZone: timezone } : {}) }).format(date)}` : day;
        } catch (_) {
            return formatDate(value, withTime);
        }
    }

    function homePreferencesStatusMarkup() {
        if (state.homePreferencesSaving) return '<p class="mobile-app-home-news__status" role="status">Enregistrement…</p>';
        if (state.homePreferencesError) return `<p class="mobile-app-home-news__status" role="status">${escapeHtml(state.homePreferencesError)} <button type="button" class="mobile-app-home-news__refresh" data-mobile-home-preferences-retry>Réessayer</button></p>`;
        if (state.homePreferencesLoading || !state.homePreferencesReady) return '<p class="mobile-app-home-news__status" role="status">Chargement du réglage…</p>';
        if (state.quickActionsEditing && (!state.homePreferences.editable || state.readOnly)) return '<p class="mobile-app-home-news__status">Ce réglage n’est pas modifiable pour ce profil.</p>';
        return '';
    }

    function homeNewsToggleMarkup() {
        if (!state.homePreferencesReady) return '';
        return `<label class="mobile-app-task-switch mobile-app-home-news__toggle"><input type="checkbox" role="switch" aria-label="Afficher Économie et fiscalité sur l’accueil" data-mobile-home-news-enabled ${state.homePreferences.news_enabled ? 'checked' : ''} ${state.homePreferencesSaving || state.homePreferencesLoading || state.quickActionsSaving || !state.homePreferences.editable || state.readOnly ? 'disabled' : ''}><span aria-hidden="true"><span></span></span></label>`;
    }

    function homeNewsMarkup() {
        const visible = state.homePreferencesReady && state.homePreferences?.news_enabled === true;
        let content = '';
        if (visible) content = homeNewsFeedMarkup();
        else if (state.quickActionsEditing || !state.homePreferencesReady) {
            content = `<section class="mobile-app-home-news" data-mobile-home-news aria-labelledby="mobile-home-news-title"><div class="mobile-app-home-news__heading"><div><h2 id="mobile-home-news-title">Économie et fiscalité</h2><p>L’actualité en France et dans le monde</p></div>${state.quickActionsEditing ? homeNewsToggleMarkup() : ''}</div>${homePreferencesStatusMarkup()}${state.homePreferencesReady && !state.homePreferences.news_enabled ? '<p class="mobile-app-home-news__status">Masqué sur l’accueil.</p>' : ''}</section>`;
        }
        return `<div data-mobile-home-news-area>${content}</div>`;
    }

    function homeNewsFeedMarkup() {
        const unavailable = 'Le fil d’actualités est momentanément indisponible. Réessayez dans quelques instants.';
        const hasArticles = Boolean(state.homeNews?.france?.length || state.homeNews?.monde?.length);
        const retry = Boolean(state.homeNewsError || (state.homeNews && !hasArticles));
        const buttonLabel = state.homeNewsLoading ? 'Actualisation…' : (retry ? 'Réessayer' : 'Actualiser');
        const groups = [['france', 'France'], ['monde', 'Dans le monde']].map(([region, label]) => {
            const articles = state.homeNews?.[region] || [];
            if (!articles.length) return '';
            const rows = articles.map((article) => {
                const url = homeNewsLink(article.url);
                const sourceUrl = homeNewsLink(article.source_url);
                const date = article.published_at ? homeNewsDate(article.published_at) : formatCivilDate(article.published_on);
                const link = (href, text, className) => href
                    ? `<a class="${className}" href="${escapeHtml(href)}" data-mobile-home-news-link target="_blank" rel="noopener noreferrer">${escapeHtml(text)}</a>`
                    : `<span class="${className}">${escapeHtml(text)}</span>`;
                return `<li>${link(url, article.title, 'mobile-app-home-news__article')}<div class="mobile-app-home-news__meta">${date ? `<time datetime="${escapeHtml(article.published_at || article.published_on)}">${escapeHtml(date)}</time><span aria-hidden="true">-</span>` : ''}${link(sourceUrl, article.source, 'mobile-app-home-news__source')}</div></li>`;
            }).join('');
            return `<section class="mobile-app-home-news__group" aria-labelledby="mobile-home-news-${region}"><h3 id="mobile-home-news-${region}">${label}</h3><ul>${rows}</ul></section>`;
        }).join('');
        const emptyMessage = state.homeNewsLoading || (!state.homeNews && !state.homeNewsError)
            ? 'Chargement des actualités…'
            : (state.homeNewsError || unavailable);
        const updated = homeNewsDate(state.homeNews?.updated_at, true);
        return `<section class="mobile-app-home-news" data-mobile-home-news aria-labelledby="mobile-home-news-title"><div class="mobile-app-home-news__heading"><div><h2 id="mobile-home-news-title">Économie et fiscalité</h2><p>L’actualité en France et dans le monde</p></div>${state.quickActionsEditing ? homeNewsToggleMarkup() : `<button type="button" class="mobile-app-home-news__refresh" data-mobile-home-news-refresh ${retry ? 'data-mobile-home-news-retry' : ''} ${state.homeNewsLoading ? 'disabled' : ''}>${buttonLabel}</button>`}</div>${homePreferencesStatusMarkup()}<div class="mobile-app-home-news__body" aria-busy="${state.homeNewsLoading}">${hasArticles ? groups : `<p class="mobile-app-home-news__status" role="status">${escapeHtml(emptyMessage)}</p>`}</div>${hasArticles && state.homeNewsError ? `<p class="mobile-app-home-news__status" role="status">${escapeHtml(state.homeNewsError)}</p>` : ''}${updated ? `<p class="mobile-app-home-news__updated">Dernière actualisation le <time datetime="${escapeHtml(state.homeNews.updated_at)}">${escapeHtml(updated)}</time></p>` : ''}</section>`;
    }

    function renderHomeNews() {
        if (state.route !== 'home') return;
        const section = app.querySelector('[data-mobile-home-news-area]');
        if (section) section.outerHTML = homeNewsMarkup();
        const controls = app.querySelector('[data-mobile-home-personalize]');
        if (controls) controls.innerHTML = homePersonalizeButton();
    }

    function stopHomePreferenceRequests() {
        state.homePreferencesSequence += 1;
        state.homePreferencesController?.abort();
        state.homePreferencesController = null;
        state.homePreferencesLoading = false;
        state.homePreferencesSaving = false;
        state.homePreferencesReady = false;
        state.homePreferencesChecked = false;
        state.homePreferencesError = '';
        state.homePreferencesRetryValue = null;
    }

    async function requestHomePreferences(newsEnabled = null) {
        const saving = typeof newsEnabled === 'boolean';
        if (!state.authenticated || state.route !== 'home' || document.hidden || state.homePreferencesLoading || state.homePreferencesSaving) return;
        if (saving && (!state.homePreferencesReady || !state.homePreferences?.editable || state.readOnly)) return;
        stopHomeNewsRefresh();
        const scope = `${mobileDraftScope()}:${state.cameleonSessionId || ''}`;
        const sequence = ++state.homePreferencesSequence;
        const controller = new AbortController();
        state.homePreferencesController = controller;
        state.homePreferencesChecked = true;
        state.homePreferencesLoading = !saving;
        state.homePreferencesSaving = saving;
        state.homePreferencesError = '';
        state.homePreferencesRetryValue = newsEnabled;
        if (!saving) state.homePreferencesReady = false;
        renderHomeNews();
        const timeout = window.setTimeout(() => controller.abort(), 15000);
        const isCurrent = () => sequence === state.homePreferencesSequence && scope === `${mobileDraftScope()}:${state.cameleonSessionId || ''}` && state.route === 'home' && state.authenticated;
        try {
            const response = await api('/home/preferences', { ...(saving ? { method: 'PATCH', body: { news_enabled: newsEnabled } } : {}), signal: controller.signal });
            if (!isCurrent()) return;
            const data = response?.data;
            if (typeof data?.news_enabled !== 'boolean' || typeof data?.editable !== 'boolean') throw new Error('Invalid home preferences response');
            state.homePreferences = { news_enabled: data.news_enabled, editable: data.editable };
            state.homePreferencesReady = true;
            state.homePreferencesRetryValue = null;
            ...
            if (!isCurrent()) return;
            state.homePreferencesError = saving
                ? 'Le réglage n’a pas pu être enregistré. Réessayez.'
                : 'Le réglage d’affichage ne peut pas être chargé. Réessayez.';
        } finally {
            window.clearTimeout(timeout);
            if (isCurrent()) {
                state.homePreferencesLoading = false;
                ...
                state.homePreferencesController = null;
                renderHomeNews();
                startHomeNewsRefresh({ force: false });
            }
        }
    }

    function stopHomeNewsRefresh() {
        if (state.homeNewsTimer !== null) window.clearInterval(state.homeNewsTimer);
        state.homeNewsTimer = null;
        state.homeNewsRequestSequence += 1;
        state.homeNewsController?.abort();
        state.homeNewsController = null;
        state.homeNewsLoading = false;
    }

    async function refreshHomeNews() {
        if (!state.authenticated || state.route !== 'home' || document.hidden || state.homeNewsLoading
            || !state.homePreferencesReady || !state.homePreferences?.news_enabled || state.homePreferencesSaving
            || state.externalApplication || state.biometricLocked || !app.querySelector('[data-mobile-home-news]')) return;
        const sequence = ++state.homeNewsRequestSequence;
        const scope = mobileDraftScope();
        const controller = new AbortController();
        state.homeNewsController = controller;
        state.homeNewsLoading = true;
        state.homeNewsError = '';
        state.homeNewsLastAttempt = Date.now();
        renderHomeNews();
        const timeout = window.setTimeout(() => controller.abort(), 15000);
        try {
            const response = await api('/home/news', { signal: controller.signal });
            if (sequence !== state.homeNewsRequestSequence || scope !== mobileDraftScope() || state.route !== 'home' || !state.authenticated) return;
            const data = response?.data;
            if (!data || !Array.isArray(data.france) || !Array.isArray(data.monde)) throw new Error('Invalid news response');
            const articles = (items, limit) => items.filter((item) => item && typeof item.title === 'string' && item.title.trim()).slice(0, limit);
            const next = { france: articles(data.france, 8), monde: articles(data.monde, 2), updated_at: data.updated_at || null, timezone: data.timezone || '' };
            if (!next.france.length && !next.monde.length && (state.homeNews?.france?.length || state.homeNews?.monde?.length)) {
                state.homeNewsError = 'Le fil d’actualités est momentanément indisponible. Réessayez dans quelques instants.';
            } else state.homeNews = next;
        } catch (_) {
            if (sequence !== state.homeNewsRequestSequence || scope !== mobileDraftScope() || state.route !== 'home' || !state.authenticated) return;
            state.homeNewsError = 'Le fil d’actualités ne peut pas être actualisé. Réessayez dans quelques instants.';
        } finally {
            window.clearTimeout(timeout);
            if (sequence === state.homeNewsRequestSequence) {
                state.homeNewsLoading = false;
                state.homeNewsController = null;
                renderHomeNews();
            }
    ...
    function startHomeNewsRefresh({ force = true } = {}) {
        if (!state.authenticated || state.route !== 'home' || document.hidden || !app.querySelector('[data-mobile-home-news-area]')) return;
        const preferenceScope = `${mobileDraftScope()}:${state.cameleonSessionId || ''}`;
        if (state.homePreferencesScope !== preferenceScope) {
            stopHomePreferenceRequests();
            state.homePreferencesScope = preferenceScope;
            state.homePreferences = null;
        }
        const scope = mobileDraftScope();
        if (state.homeNewsScope !== scope) {
            stopHomeNewsRefresh();
            state.homeNewsScope = scope;
            state.homeNews = null;
            state.homeNewsError = '';
            state.homeNewsLastAttempt = 0;
        }
        if (!state.homePreferencesChecked) {
            void requestHomePreferences();
            return;
        }
        if (!state.homePreferencesReady || !state.homePreferences?.news_enabled || state.homePreferencesSaving) return;
        if (state.homeNewsTimer !== null) return;
        state.homeNewsTimer = window.setInterval(() => { void refreshHomeNews(); }, 15 * 60 * 1000);
        if (force || !state.homeNews || Date.now() - state.homeNewsLastAttempt >= 15 * 60 * 1000) void refreshHomeNews();
        else renderHomeNews();
    }

    function homeFollowUpMarkup(items, title) {
        if (!items.length) return '';
        const rows = items.map((item) => {
            const iconKey = ({ tasks: 'tasks', tasks_overdue: 'tasks', absences: 'absence', absences_submitted: 'absence', expenses: 'expense', expenses_pending: 'expense', expense_draft: 'expense' })[item.key] || 'expense';
            return `<button class="mobile-app-home-follow-up__item" type="button" data-mobile-home-destination="${escapeHtml(item.destination || '')}"><span class="mobile-app-home-follow-up__icon">${quickActionIcons[iconKey]}</span><span class="mobile-app-home-follow-up__copy"><strong>${escapeHtml(item.title || '')}</strong>${item.subtitle ? `<small>${escapeHtml(item.subtitle)}</small>` : ''}</span><svg class="mobile-app-home-follow-up__chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="m9 6 6 6-6 6-1.4-1.4 4.6-4.6-4.6-4.6L9 6Z"/></svg></button>`;
        }).join('');
        return `<section class="mobile-app-home-follow-up" aria-label="${escapeHtml(title)}"><h2>${escapeHtml(title)}</h2><div class="mobile-app-home-follow-up__list">${rows}</div></section>`;
    }
    ...
    function renderQuickActionsHome() {
        ...
        const actionsEditing = state.quickActionsEditing && state.homeQuickActionsEditable;
        if (state.homeQuickActionScope) {
            if (actionsEditing) {
                const draft = { keys: [...state.homeQuickActionKeys] };
                state.homeQuickActionDrafts[state.homeQuickActionScope] = draft;
                const key = `home:${state.homeQuickActionScope}`;
                const value = JSON.stringify(draft);
                if (secureMobileDrafts.get(key) !== value) writeSecureMobileDraft(key, value);
            }
            else if (state.homeQuickActionsEditable) delete state.homeQuickActionDrafts[state.homeQuickActionScope];
        }
        const editButton = `<span data-mobile-home-personalize>${homePersonalizeButton()}</span>`;
        const empty = actions.length === 0
            ? '<div class="mobile-app-quick-actions-empty">Aucune action rapide n’est affichée.</div>'
            : '';
        const attention = state.quickActionsEditing ? '' : homeFollowUpMarkup(state.homeAttentionItems, 'À suivre');
        const resume = !state.quickActionsEditing && state.homeResumeAction ? homeFollowUpMarkup([state.homeResumeAction], 'Reprendre') : '';
        const content = `${attention}<section class="mobile-app-home-actions"><div class="mobile-app-section-title"><h2>Actions rapides</h2>${editButton}</div>
            ${actionsEditing ? '<p class="mobile-app-quick-actions-hint">Vos raccourcis pour ce dossier.</p><p class="mobile-app-quick-actions-hint">Maintenez une action appuyée puis faites-la glisser pour la déplacer.</p>' : ''}
            <div class="mobile-app-action-row mobile-app-home-grid ${actionsEditing ? 'is-editing' : ''}">${actions.map(quickActionMarkup).join('')}</div>
            ${empty}
            ${actionsEditing ? `${state.quickActionsError ? `<p class="mobile-app-quick-actions-error" role="alert">${escapeHtml(state.quickActionsError)}</p>` : ''}<button class="mobile-app-quick-actions-add" type="button" data-quick-actions-add ${state.quickActionsSaving ? 'disabled' : ''}>+ Ajouter une action</button><button class="mobile-app-quick-actions-edit mobile-app-home-restore" type="button" data-quick-actions-restore ${state.quickActionsSaving ? 'disabled' : ''}>Rétablir les actions par défaut</button>` : ''}
            </section>${resume}${homeNewsMarkup()}${quickActionDialogMarkup()}`;
    ...
    function documentFilterParams(params = state.routeParams || {}) {
        return Object.fromEntries(['type', 'from', 'to'].filter((key) => params[key]).map((key) => [key, String(params[key])]));
    }

    function documentSelectionItems() {
        const files = state.paginatedLists.documents?.items || [];
        return [...state.selectedDocumentItems].map((key) => {
            const [type, id] = key.split(':');
            return type === 'document' ? files.find((file) => Number(file.id) === Number(id)) : null;
        });
    }

    function documentSelectionAllowed(operation) {
        const selected = documentSelectionItems();
        return selected.length > 0 && selected.length <= 50 && selected.every((file) => file?.[`can_${operation}`] === true)
            && (operation !== 'move' || !state.readOnly);
    }

    function documentDialog(title, body) {
        app.querySelector('[data-document-tools-dialog]')?.remove();
        app.insertAdjacentHTML('beforeend', `<div class="mobile-app-dialog-backdrop" data-document-tools-dialog><section class="mobile-app-quick-actions-dialog" role="dialog" aria-modal="true" aria-labelledby="mobile-document-tools-title"><header><h3 id="mobile-document-tools-title">${escapeHtml(title)}</h3><button type="button" data-document-tools-close aria-label="Fermer">×</button></header><div class="mobile-app-quick-actions-dialog__body">${body}<p class="mobile-app-form-error" data-document-tools-error role="alert" hidden></p></div></section></div>`);
        enhanceMobileSelects(app.querySelector('[data-document-tools-dialog]'));
    }

    function openDocumentFilters() {
        const filters = documentFilterParams();
        const choices = [['', 'Tous les fichiers'], ['pdf', 'PDF'], ['image', 'Images'], ['spreadsheet', 'Tableurs'], ['text', 'Textes'], ['archive', 'Archives']];
        documentDialog('Filtrer les fichiers', `<div class="mobile-app-field"><label>Type</label><select data-document-filter-type>${choices.map(([value, label]) => `<option value="${value}" ${filters.type === value ? 'selected' : ''}>${label}</option>`).join('')}</select></div><p class="mobile-app-form-hint">Date d’ajout</p><div class="mobile-app-form-row"><div class="mobile-app-field"><label>Du</label><input type="date" data-document-filter-from value="${escapeHtml(filters.from || '')}"></div><div class="mobile-app-field"><label>Au</label><input type="date" data-document-filter-to value="${escapeHtml(filters.to || '')}"></div></div><div class="mobile-app-module-actions"><button class="mobile-app-secondary-button" type="button" data-document-filters-reset>Réinitialiser</button><button class="mobile-app-primary-button" type="button" data-document-filters-apply>Appliquer</button></div>`);
    }

    async function openDocumentMove() {
        if (!documentSelectionAllowed('move')) return;
        const scope = mobileDataScope();
        const personal = state.routeParams?.scope === 'personal';
        const endpoint = personal ? '/personal-documents' : `/dossiers/${state.activeDossierId}/documents`;
        const response = await api(`${endpoint}?per_page=1&tree_scope=all`, { expectedIdentityScope: scope });
        if (scope !== mobileDataScope() || state.route !== 'documents') return;
        const folders = response.meta?.document_folders || [];
        documentDialog('Déplacer les fichiers', `<p class="mobile-app-form-hint">${state.selectedDocumentItems.size} ${state.selectedDocumentItems.size === 1 ? 'fichier sélectionné' : 'fichiers sélectionnés'}</p><div class="mobile-app-field"><label>Dossier de destination</label><select data-document-move-folder>${folders.map((folder) => `<option value="${Number(folder.id)}">${escapeHtml(folder.path || folder.name)}</option>`).join('')}</select></div><button class="mobile-app-primary-button" type="button" data-document-move-confirm>Déplacer</button>`);
    }

    async function applyDocumentSelection(operation, trigger) {
        if (!documentSelectionAllowed(operation) || state.documentSelectionBusy) return;
        state.documentSelectionBusy = true;
        const scope = mobileDataScope();
        const selected = documentSelectionItems();
        const endpoint = state.routeParams?.scope === 'personal' ? '/personal-documents' : `/dossiers/${state.activeDossierId}/documents`;
        const folderId = Number(app.querySelector('[data-document-move-folder]')?.value) || 0;
        trigger.disabled = true;
        syncDocumentSelectionUi();
        const body = { operation, ids: selected.map((file) => Number(file.id)), ...(operation === 'move' ? { folder_id: folderId } : {}) };
        const signature = `${scope}:${JSON.stringify(body)}`;
        if (state.documentSelectionOperation?.signature !== signature) state.documentSelectionOperation = { signature, key: crypto.randomUUID() };
        try {
            const response = await api(`${endpoint}/selection`, { method: 'POST', expectedIdentityScope: scope,
                idempotencyKey: state.documentSelectionOperation.key, body });
            state.documentSelectionOperation = null;
            if (scope !== mobileDataScope() || state.route !== 'documents') return;
            const results = response.data?.results || [];
            const failed = results.filter((item) => !item.ok);
            if (operation === 'move') {
                state.selectedDocumentItems = new Set(failed.map((item) => documentSelectionKey('document', item.id)));
                state.documentSelectionMode = failed.length > 0;
                await renderDocuments(state.routeParams || {});
            }
            const rows = results.map((result) => `<div class="mobile-app-detail-line"><strong>${escapeHtml(selected.find((file) => Number(file.id) === Number(result.id))?.title || 'Fichier')}</strong><p>${escapeHtml(result.message)}</p></div>`).join('');
            const share = operation === 'share' && response.data?.content_url
                ? `<button class="mobile-app-primary-button" type="button" data-share-binary="${escapeHtml(response.data.content_url)}" data-share-filename="documents.zip" data-share-mime="application/zip">Partager ${results.filter((item) => item.ok).length === 1 ? 'le fichier' : 'les fichiers'} (ZIP)</button>` : '';
            documentDialog(operation === 'move' ? 'Résultat du déplacement' : 'Fichiers à partager', `${rows}${share}`);
        } catch (error) {
            const message = error?.message || 'L’opération a échoué. Réessayez.';
            const target = app.querySelector('[data-document-tools-error]');
            if (target) { target.textContent = message; target.hidden = false; }
            else showQuickCreationNotice(message, 6000);
        } finally {
            state.documentSelectionBusy = false;
    ...
    function documentSelectionToolbarMarkup() {
        ...
        return `<div class="mobile-app-document-selection-toolbar" data-mobile-document-selection-toolbar ${state.documentSelectionMode ? '' : 'hidden'}><button type="button" data-mobile-document-selection-close aria-label="Quitter la sélection">×</button><strong data-mobile-document-selection-count>${escapeHtml(label)}</strong><button type="button" data-document-selection-share aria-label="Partager la sélection" title="Partager" disabled>${quickActionIcons.share}</button><button type="button" data-document-selection-move aria-label="Déplacer la sélection" title="Déplacer" disabled>${documentScopeIcons.current}</button><button type="button" class="mobile-app-document-selection-delete" data-mobile-document-selection-delete aria-label="Supprimer la sélection" title="Supprimer la sélection" ${count > 0 && !state.documentDeletionBusy ? '' : 'disabled'}>${quickActionIcons.delete}</button></div>`;
    }

    function documentListMarkup(items, childFolders = [], showFolderPath = false, hasSearch = false, personalDocuments = false, recent = false) {
        const folderItems = (Array.isArray(childFolders) ? childFolders : []).map((folder) => {
            ...
            return `<article class="mobile-app-document-folder-card ${state.selectedDocumentItems.has(key) ? 'is-selected' : ''}" data-mobile-document-selectable="${escapeHtml(key)}" data-mobile-document-can-delete="${folder.can_delete ? '1' : '0'}" data-mobile-document-can-select="${folder.can_delete ? '1' : '0'}">${documentSelectionCheckbox('folder', id, folder.can_delete === true)}<button class="mobile-app-document-folder-card__open" type="button" data-mobile-document-child-folder="${id}" aria-label="Ouvrir le dossier ${escapeHtml(name)}"><span class="mobile-app-document-folder-card__icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M3 5h7l2 2h9v12H3V5Zm2 4v8h14V9H5Z"/></svg></span><strong title="${escapeHtml(name)}">${escapeHtml(name)}</strong></button></article>`;
        }).join('');
        ...
        const documentMarkup = (Array.isArray(items) ? items : []).map((document) => {
                ...
                const selectable = canDelete || document.can_share === true || document.can_move === true;
                return `<article class="mobile-app-list-item mobile-app-document-item ${state.selectedDocumentItems.has(key) ? 'is-selected' : ''}" data-mobile-document-selectable="${escapeHtml(key)}" data-mobile-document-can-delete="${canDelete ? '1' : '0'}" data-mobile-document-can-select="${selectable ? '1' : '0'}">
                    ${documentSelectionCheckbox('document', id, selectable)}
                    ...
                    <span class="mobile-app-document-actions">
                        ...
                        ${document.can_share === true ? `<button type="button" data-share-binary="${escapeHtml(document.content_url + (document.content_url.includes('?') ? '&' : '?') + 'share=1')}" data-share-filename="${escapeHtml(filename)}" data-share-mime="${escapeHtml(mimeType)}" data-share-company="${escapeHtml(companyName)}" aria-label="Partager ${escapeHtml(document.title || filename)}" title="Partager">${quickActionIcons.share}</button>` : ''}
                    </span>
        ...
        return folderMarkup || documentMarkup
            ...
            : emptyState('Aucun document', hasSearch
                ? 'Aucun document autorisé ne correspond à ces critères.'
                : recent ? 'Aucun fichier récemment consulté dans cette entreprise.'
                : personalDocuments
    ...
    function documentSearchScopeMarkup(scope) {
        const labels = {
            ...
            personal: 'Mode personnel - appuyer pour afficher les fichiers récents',
            recent: 'Mode Récents - appuyer pour revenir au dossier courant',
        };
    ...
    async function loadDocumentSearch(query, scope, requestSequence) {
        ...
        try {
            ...
            const parameters = new URLSearchParams({ folder_id: String(folderId), per_page: '20', tree_scope: 'current' });
            parameters.set('sort', state.documentSortMode);
            if (query) parameters.set('q', query);
            if (allFolders) parameters.set('search_scope', 'all');
            if (scope === 'recent') parameters.set('recent', '1');
            Object.entries(documentFilterParams()).forEach(([key, value]) => parameters.set(key, value));
            ...
            results.innerHTML = preparePaginatedList('documents', `${endpoint}?${parameters}`, response, response.data || [], (items) => documentListMarkup(items, childFolders, allFolders || personalDocuments, query.length > 0 || Object.keys(documentFilterParams()).length > 0, personalDocuments, scope === 'recent'));
    ...
    async function renderDocuments(params = {}) {
        const requestedScope = mobileDataScope();
        const navigationGeneration = state.navigationGeneration;
        const search = String(params.q || '').slice(0, 120);
        const scope = ['all', 'personal', 'recent'].includes(params.scope) ? params.scope : 'current';
        const requestedFolderId = Math.max(0, Number(params.folder) || 0);
        const allFolders = ['all', 'recent'].includes(scope);
        const personalDocuments = scope === 'personal';
        const parameters = new URLSearchParams({ folder_id: String(requestedFolderId), per_page: '20', tree_scope: 'current' });
        parameters.set('sort', state.documentSortMode);
        if (search) parameters.set('q', search);
        if (allFolders) parameters.set('search_scope', 'all');
            if (scope === 'recent') parameters.set('recent', '1');
            Object.entries(documentFilterParams(params)).forEach(([key, value]) => parameters.set(key, value));
        const endpoint = personalDocuments
            ? '/personal-documents'
            : `/dossiers/${state.activeDossierId}/documents`;
        const response = await api(`${endpoint}?${parameters.toString()}`);
        if (state.route !== 'documents' || requestedScope !== mobileDataScope() || navigationGeneration !== state.navigationGeneration) return false;
        ...
        state.documentFolderTreeComplete = response.meta?.document_tree_scope !== 'current';
        state.routeParams = {
            ...documentFilterParams(params),
            ...(folderId > 0 ? { folder: String(folderId) } : {}),
            ...(search ? { q: search } : {}),
            ...(scope !== 'current' ? { scope } : {}),
        };
        persistNavigation();
        const recentHint = scope === 'recent' ? '<p class="mobile-app-form-hint">Fichiers de cette entreprise consultés au cours des 180 derniers jours.</p>' : '';
        const list = recentHint + preparePaginatedList('documents', `${endpoint}?${parameters}`, response, items, (rows) => documentListMarkup(rows, childFolders, allFolders || personalDocuments, search.length > 0 || Object.keys(documentFilterParams(params)).length > 0, personalDocuments, scope === 'recent'));
    ...
    function syncDocumentSelectionUi() {
        ...
        if (deleteButton) deleteButton.disabled = !canDeleteCurrentDocuments() || count === 0 || state.documentDeletionBusy
            || [...state.selectedDocumentItems].some((key) => app.querySelector(`[data-mobile-document-selectable="${CSS.escape(key)}"]`)?.dataset.mobileDocumentCanDelete !== '1');
        const share = app.querySelector('[data-document-selection-share]');
        const move = app.querySelector('[data-document-selection-move]');
        if (share) share.disabled = !documentSelectionAllowed('share') || state.documentSelectionBusy;
        if (move) move.disabled = !documentSelectionAllowed('move') || state.documentSelectionBusy;
    }
    ...
    function enterDocumentSelectionMode(item) {
        if (!(item instanceof HTMLElement) || item.dataset.mobileDocumentCanSelect !== '1') return;
    ...
    function hrPreferenceKey() { const profile = state.bootstrap?.profile || {}; return `${Number(profile.owner_user_id || profile.id) || 0}:${Number(profile.id) || 0}:${state.activeDossierId}`; }

    function rememberHrSection(route) {
        if (!managerHrEntries().some(([key]) => key === route)) return;
        state.hrLastSections[hrPreferenceKey()] = route;
        persistPreferences();
    }

    function hrStatusFilterMarkup(status) {
        const labels = { submitted: 'En attente', approved_unpaid: 'Validés - paiement en attente', paid: 'Payés', draft: 'À compléter' };
        return status !== 'all' ? `<div class="mobile-app-compact-summary"><span>${escapeHtml(labels[status] || status)}</span><button class="mobile-app-text-button" type="button" data-hr-clear-status>Tout afficher</button></div>` : '';
    }

    function hrOverviewMarkup() {
        return `<section class="mobile-app-hr-overview"><button class="mobile-app-hr-overview__toggle" type="button" data-hr-overview-toggle aria-expanded="false"><strong>Ma situation</strong><span aria-hidden="true">⌄</span></button><div class="mobile-app-hr-overview__body" data-hr-overview-body hidden></div></section>`;
    }

    async function loadHrOverview(month = '', page = 1) {
        const body = app.querySelector('[data-hr-overview-body]');
        if (!body || body.hidden) return;
        const scope = mobileDataScope();
        const generation = state.navigationGeneration;
        body.setAttribute('aria-busy', 'true');
        try {
            const query = new URLSearchParams({ page: String(page), ...(month ? { month } : {}) });
            const response = await api(`/dossiers/${state.activeDossierId}/hr/overview?${query}`, { expectedIdentityScope: scope });
            if (scope !== mobileDataScope() || generation !== state.navigationGeneration || !body.isConnected) return;
            const data = response.data || {};
            const balance = data.balance;
            const balanceMarkup = balance ? `<button type="button" class="mobile-app-hr-overview__card" data-mobile-route="leave_balances"><span>Solde CP</span><strong>${balance.initialized && balance.available !== null ? escapeHtml(formatLeaveDays(balance.available)) : 'Non initialisé'}</strong></button>` : '';
            const cards = (data.cards || []).map((card) => `<button type="button" class="mobile-app-hr-overview__card" data-hr-overview-route="${escapeHtml(card.route)}" data-hr-overview-filter="${escapeHtml(card.key)}"><span>${escapeHtml(card.label)}</span><strong>${Number(card.count)}</strong></button>`).join('');
            const drafts = (data.drafts || []).map((draft) => `<button class="mobile-app-list-button" type="button" data-hr-resume-draft="${Number(draft.id)}">Reprendre - ${escapeHtml(draft.label)}</button>`).join('');
            const calendar = data.calendar;
            const events = calendar?.absences || [];
            const agenda = calendar ? `<div class="mobile-app-hr-agenda"><div class="mobile-app-hr-agenda__month"><button type="button" data-hr-overview-month="${calendar.previous_month}" aria-label="Mois précédent">‹</button><strong>${escapeHtml(new Intl.DateTimeFormat('fr-FR', { month: 'long', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${calendar.month}-01T12:00:00Z`)))}</strong><button type="button" data-hr-overview-month="${calendar.next_month}" aria-label="Mois suivant">›</button></div><p class="mobile-app-form-hint">Agenda des absences - ${Number(calendar.pagination?.total)} ${Number(calendar.pagination?.total) === 1 ? 'demande' : 'demandes'}</p>${events.map((item) => `<button class="mobile-app-hr-agenda__item" type="button" data-hr-agenda-absence="${Number(item.id)}"><strong>${escapeHtml(item.employee?.display_name || item.leave_type?.label || 'Absence')}</strong><span>${formatCivilDate(item.starts_on)} - ${formatCivilDate(item.ends_on)}</span><small>${escapeHtml(item.status_label || humanStatus(item.status))}</small></button>`).join('') || '<p class="mobile-app-form-hint">Aucune absence en attente ou validée sur ce mois.</p>'}${Number(calendar.pagination?.last_page) > 1 ? `<div class="mobile-app-hr-agenda__month"><button type="button" data-hr-overview-month="${calendar.month}" data-hr-overview-page="${page - 1}" ${page > 1 ? '' : 'disabled'}>Précédent</button><span>${page} / ${Number(calendar.pagination.last_page)}</span><button type="button" data-hr-overview-month="${calendar.month}" data-hr-overview-page="${page + 1}" ${calendar.pagination.has_more ? '' : 'disabled'}>Suivant</button></div>` : ''}</div>` : '';
            body.innerHTML = `${data.mode === 'validator' ? '<p class="mobile-app-form-hint">Salariés dont vous êtes valideur.</p>' : ''}<div class="mobile-app-hr-overview__cards">${balanceMarkup}${cards}</div>${drafts}${agenda}`;
        } catch (error) {
            if (scope === mobileDataScope() && body.isConnected) body.innerHTML = `<p class="mobile-app-form-error" role="alert">${escapeHtml(error?.message || 'Synthèse indisponible.')}</p><button class="mobile-app-secondary-button" type="button" data-hr-overview-retry>Réessayer</button>`;
        } finally { body.removeAttribute('aria-busy'); }
    }

    function hrListMarkup(key, url, response, items, render, pending = false) {
        const total = Number(response.meta?.pagination?.total);
        const count = Number.isFinite(total) ? `<p class="mobile-app-form-hint">${total} ${total === 1 ? 'demande' : 'demandes'}</p>` : '';
        return preparePaginatedList(key, url, response, items, (rows) => `${count}<div class="mobile-app-list ${pending ? 'mobile-app-validation-list' : ''}">${rows.map(render).join('')}</div>`);
    }

    function hrPendingFailure(error, route) {
        return `<div class="mobile-app-settings-message is-error" role="alert">${escapeHtml(error?.message || 'Les demandes à valider ne sont pas disponibles.')}<button class="mobile-app-quick-actions-edit" type="button" data-mobile-route="${route}" data-mobile-retry-params="${escapeHtml(JSON.stringify(state.routeParams || {}))}">Réessayer</button></div>`;
    }

    async function openHrLinkedDetail(kind, id) {
        try { if (kind === 'expense') await openExpenseDetail(id); else await openAbsenceDetail(id); }
        catch (error) { documentDialog('Demande indisponible', `<p role="alert">${escapeHtml([403, 404].includes(Number(error?.status)) ? 'Cette demande a été supprimée ou vos droits ne permettent plus de la consulter.' : (error?.message || 'Impossible de charger cette demande.'))}</p>`); }
    }

    async function performHrTransition(trigger, reject = false) {
        if (trigger.disabled || trigger.dataset.hrBlocked === 'true' || state.readOnly) return;
        const kind = trigger.dataset.hrKind === 'expense' ? 'expense' : 'absence';
        const id = Number(trigger.dataset.hrId);
        const scope = mobileDataScope();
        const route = state.route;
        const generation = state.navigationGeneration;
        const path = `/dossiers/${state.activeDossierId}/${kind === 'expense' ? 'expense-reports' : 'absences'}/${id}`;
        const host = trigger.closest('.mobile-app-quick-actions-dialog__body, .mobile-app-list-item') || trigger.parentElement;
        let errorNode = host.querySelector('[data-hr-operation-error]');
        if (!errorNode) { errorNode = document.createElement('p'); errorNode.className = 'mobile-app-form-error'; errorNode.dataset.hrOperationError = ''; errorNode.setAttribute('role', 'alert'); host.append(errorNode); }
        errorNode.textContent = '';
        const body = { transition: reject ? 'reject' : String(trigger.dataset.hrTransition || 'approve'), lock_version: Number(trigger.dataset.hrLockVersion) || 0 };
        if (reject) { body.comment = String(app.querySelector('[data-hr-reject-reason]')?.value || '').trim(); if (!body.comment) return; }
        const key = `${scope}:${kind}:${id}`;
        state.hrOperations ||= new Map();
        if (state.hrOperations.get(key)?.busy) return;
        const previous = state.hrOperations.get(key);
        const signature = JSON.stringify(body);
        const operation = { busy: true, signature, idempotency: previous?.signature === signature ? previous.idempotency : crypto.randomUUID() };
        state.hrOperations.set(key, operation);
        const buttons = [...app.querySelectorAll(`[data-hr-id="${id}"]`)].filter((button) => (button.dataset.hrKind || button.dataset.hrReject) === kind);
        const enabled = buttons.filter((button) => !button.disabled);
        buttons.forEach((button) => { button.disabled = true; button.dataset.hrBusy = 'true'; button.setAttribute('aria-busy', 'true'); });
        let acknowledged = false;
        let conflictAllowed = true;
        try {
            await api(`${path}/transition`, { method: 'POST', body, expectedIdentityScope: scope, idempotencyKey: operation.idempotency });
            acknowledged = true;
            if (scope !== mobileDataScope() || generation !== state.navigationGeneration) return;
            app.querySelector('[data-hr-reject-dialog]')?.remove();
            if (kind === 'expense') closeExpenseDetailDialog(); else app.querySelector('[data-absence-detail-dialog]')?.remove();
            const params = { ...(state.routeParams || {}) };
            delete params[kind === 'expense' ? 'reportId' : 'absenceId'];
            if (route === 'hr_pending') { await loadHrPendingValidations(); await renderHrPendingValidations(); }
            else if (kind === 'expense') await renderExpenses(params); else await renderAbsences(params);
        } catch (error) {
            if (scope !== mobileDataScope() || generation !== state.navigationGeneration) return;
            errorNode.textContent = acknowledged ? 'Décision enregistrée. Actualisez la liste pour afficher le nouvel état.' : (error?.message || 'La décision n’a pas été enregistrée. Réessayez.');
            if (!acknowledged && Number(error?.status) === 409) {
                conflictAllowed = false;
                try {
                    const fresh = (await api(path, { expectedIdentityScope: scope })).data || {};
                    if (scope !== mobileDataScope() || !host.isConnected) return;
                    conflictAllowed = reject ? fresh.can_reject === true : fresh.can_approve === true;
                    buttons.forEach((button) => { button.dataset.hrLockVersion = String(fresh.lock_version || 0); });
                    const description = kind === 'expense' ? `${fresh.title || 'Note de frais'} - ${formatMoney(fresh.requested_total)}` : `${formatCivilDate(fresh.starts_on)} - ${formatCivilDate(fresh.ends_on)}`;
                    errorNode.textContent = `${description} - ${fresh.status_label || humanStatus(fresh.status)}. ${conflictAllowed ? 'La demande a changé. Vérifiez ces informations avant de confirmer à nouveau.' : 'La demande ne peut plus être validée ni refusée.'}`;
                    const card = app.querySelector(`[data-${kind === 'expense' ? 'expense' : 'absence'}-detail="${id}"]`);
                    if (card && !card.closest('[role="dialog"]')) {
                        const message = errorNode.textContent;
                        const inCard = card.contains(errorNode);
                        card.outerHTML = kind === 'expense' ? mobileExpenseItemMarkup(fresh, true) : mobileAbsenceItemMarkup(fresh, true);
                        if (inCard) {
                            const replacement = app.querySelector(`[data-${kind === 'expense' ? 'expense' : 'absence'}-detail="${id}"]`);
                            replacement?.insertAdjacentHTML('beforeend', `<p class="mobile-app-form-error" data-hr-operation-error role="alert">${escapeHtml(message)}</p>`);
                        }
                    }
                } catch (_) { errorNode.textContent = 'La demande a changé et son actualisation a échoué. Rechargez la liste avant de décider.'; }
            }
        } finally {
            operation.busy = false;
            buttons.forEach((button) => { delete button.dataset.hrBusy; button.dataset.hrBlocked = String(!conflictAllowed); button.removeAttribute('aria-busy'); });
            if (!acknowledged && conflictAllowed) enabled.forEach((button) => { if (button.isConnected) button.disabled = false; });
        }
    }

    function managerHrEntries() {
        return [
            ['hr_pending', 'À valider', can('absences.approve') || can('expenses.approve')],
    ...
    function managerHrPageSelectorMarkup(activeRoute) {
        ...
        return `<div class="mobile-app-form-panel mobile-app-hr-page-selector"><div class="mobile-app-field"><label for="mobile-hr-page">Rubrique</label><select id="mobile-hr-page" data-mobile-hr-page-selector>${entries.map(([route, label]) => `<option value="${route}" ${route === activeRoute ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('')}</select></div></div>`;
    }

    async function renderHr(params = {}) {
        const entries = managerHrEntries();
        if (entries.length) {
            const requestedSection = String(params.section || state.hrLastSections[hrPreferenceKey()] || '').trim();
    ...
    async function loadHrPendingValidations(more = false) {
        if (!(can('absences.approve') || can('expenses.approve'))) return null;
        const scope = mobileDataScope();
        const page = more ? Number(state.hrPendingValidations?.pagination?.current_page || 1) + 1 : 1;
        const response = await api(`/dossiers/${state.activeDossierId}/hr/pending-validations?per_page=20&page=${page}`, { expectedIdentityScope: scope });
        if (scope !== mobileDataScope()) return null;
        const next = response.data || {};
        if (more) for (const key of ['absences', 'expense_reports']) {
            const previous = state.hrPendingValidations?.[key] || [];
            const merged = new Map(previous.map((item) => [Number(item.id), item]));
            (next[key] || []).forEach((item) => merged.set(Number(item.id), item));
            next[key] = [...merged.values()];
        }
        state.hrPendingValidations = next;
        state.hrPendingError = null;
        state.hrPendingValidationCount = Number(next.total_count);
        return next;
    }

    async function renderHrPendingValidations() {
        if (!state.hrPendingValidations && !state.hrPendingError) {
            try { await loadHrPendingValidations(); } catch (error) { state.hrPendingError = error; }
        }
        const pending = state.hrPendingValidations;
        if (!pending) { renderFrame('À valider', hrPendingFailure(state.hrPendingError, 'hr_pending'), { tab: 'hr' }); return; }
        const absences = pending.absences || [];
        const expenseReports = pending.expense_reports || [];
        ...
        const group = (title, items, markup) => items.length ? `<section class="mobile-app-validation-group"><div class="mobile-app-validation-section-title"><h2>${escapeHtml(title)}</h2></div><div class="mobile-app-list mobile-app-validation-list">${items.map(markup).join('')}</div></section>` : '';
        const count = Number(pending.total_count);
        const content = count === 0 ? emptyState('Aucune demande à valider', 'Toutes les demandes accessibles ont été traitées.') : [
            `<p class="mobile-app-form-hint">${count} ${count === 1 ? 'demande à valider' : 'demandes à valider'}</p>`,
            ...
            pending.pagination?.has_more ? '<button class="mobile-app-secondary-button mobile-app-load-more" type="button" data-hr-pending-more>Charger la suite</button>' : '',
        ].join('');
        renderFrame('À valider', `${state.hrPendingError ? hrPendingFailure(state.hrPendingError, 'hr_pending') : ''}${content}`, { tab: 'hr' });
    }
    ...
    function stopNotificationPreferencesPolling() {
        state.notificationPreferencesRefreshController?.abort();
        state.notificationPreferencesRefreshController = null;
    ...
    async function refreshNotificationPreferences() {
        ...
        const controller = new AbortController();
        state.notificationPreferencesRefreshController = controller;
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const listSequence = state.notificationListSequence;
        const revision = state.notificationPreferencesRevision;
        try {
            const response = await api(notificationPreferencesEndpoint(), { signal: controller.signal, expectedIdentityScope: scope });
            if (controller.signal.aborted || state.notificationPreferencesRefreshController !== controller
                || scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration
                || listSequence !== state.notificationListSequence || state.route !== 'notifications'
                || !state.notificationPreferencesOpen || revision !== state.notificationPreferencesRevision
                || state.notificationPreferencesSaving) return;
            ...
            if (state.notificationPreferencesRefreshController === controller) {
                state.notificationPreferencesRefreshController = null;
                state.notificationPreferencesRefreshInFlight = false;
            }
        }
    ...
    function stopNotificationVisibilityTracking() {
        state.notificationVisibilityCleanup?.();
        state.notificationVisibilityCleanup = null;
    }

    function startNotificationVisibilityTracking() {
        stopNotificationVisibilityTracking();
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const listSequence = state.notificationListSequence;
        const container = app.querySelector('[data-mobile-paginated-list="notifications"]');
        const viewport = app.querySelector('.mobile-app-content');
        if (!container || !viewport) return;
        const samePage = () => scope === currentUploadIdentityScope()
            && generation === state.navigationGeneration && listSequence === state.notificationListSequence
            && state.route === 'notifications';
        const pageFocused = () => {
            if (iosNativeRuntime) return true;
            try { return window.top.document.hasFocus(); } catch (_) { return document.hasFocus(); }
        };
        state.notificationVisibilityCleanup = observeVisibleNotifications({
            container,
            viewport,
            isActive: () => {
                if (!samePage() || !state.authenticated || state.readOnly || state.offline || state.offlineCacheActive
                    || document.hidden || !pageFocused() || state.applicationBackgrounded
                    || state.biometricLocked || state.notificationPreferencesOpen) return false;
                if (iosNativeRuntime && window.__MAISON_PILOTE_IOS_CONTENT_VISIBLE__ !== true) return false;
                return ![...app.querySelectorAll('[role="dialog"], [role="menu"]:not([hidden]), [data-mobile-cameleon-menu]:not([hidden])')]
                    .some((element) => element.getClientRects().length > 0 && getComputedStyle(element).visibility !== 'hidden');
            },
            onRead: async (ids, signal) => {
                const response = await api('/notifications/read-visible', {
                    method: 'POST', body: { notification_ids: ids }, signal,
                    expectedIdentityScope: scope, offlineCache: false,
                });
                const data = response.data;
                if (!Array.isArray(data?.read_ids) || data.read_ids.length !== ids.length
                    || data.read_ids.some((id) => !Number.isSafeInteger(id) || !ids.includes(id))
                    || new Set(data.read_ids).size !== ids.length
                    || !Number.isInteger(data.unread_count) || data.unread_count < 0) throw new Error('Lecture non confirmée.');
                if (!samePage() || signal.aborted) return [];
                const read = new Set(data.read_ids);
                state.notificationItems.forEach((item) => { if (read.has(Number(item.id))) item.is_read = true; });
                container.querySelectorAll('[data-notification-visible]').forEach((card) => {
                    if (read.has(Number(card.dataset.notificationVisible))) {
                        card.dataset.notificationRead = '1';
                        const status = card.querySelector('[data-notification-read-status]');
                        if (status) status.textContent = 'Lue';
                    }
                });
                // A badge request started before this acknowledgment cannot restore an old total.
                state.notificationBadgeOperation = null;
                state.notificationBadgeScope = scope;
                state.notificationUnreadCount = data.unread_count;
                state.notificationBadgeStatus = 'loaded';
                state.notificationBadgeCheckedAt = Date.now();
                syncNotificationBadges();
    ...
    function notificationListMarkup(notificationItems) {
        ...
        const list = items.length ? `<div class="mobile-app-notification-groups">${[...grouped.values()].map((group) => `<section class="mobile-app-notification-group"><h2>${escapeHtml(group.label)}</h2>${group.items.length ? `<div class="mobile-app-list mobile-app-notifications-list">${group.items.map((item) => `<article class="mobile-app-list-item mobile-app-notification-item" data-notification-visible="${escapeHtml(item.id)}" data-notification-read="${item.is_read === true ? '1' : '0'}"><button class="mobile-app-notification-open" type="button" data-notification-open="${escapeHtml(item.id)}" ${mobileCardInteractionAttributes(item.interaction)}><span class="visually-hidden" data-notification-read-status>${item.is_read === true ? 'Lue' : 'Non lue'}</span><span class="mobile-app-list-item__top"><h3>${escapeHtml(item.title)}</h3><time datetime="${escapeHtml(item.created_at_utc || '')}">${escapeHtml(formatDate(item.created_at_utc, true))}</time></span><span>${escapeHtml(item.body)}</span></button></article>`).join('')}</div>` : state.paginatedLists.notifications?.pagination?.has_more ? '' : '<p class="mobile-app-notification-group__empty">Aucune notification</p>'}</section>`).join('')}</div>` : emptyState('Aucune notification', 'Aucune notification à afficher dans cet historique.');
        return list;
    }
    ...
    function renderNotificationsContent() {
        ...
        const list = state.paginatedLists.notifications;
        if (list && list.scope === mobileDataScope()) list.items = state.notificationItems;
        const content = list && list.scope === mobileDataScope() ? paginationMarkup('notifications') : notificationListMarkup(state.notificationItems);
        renderFrame('Historique des notifications', `${settings}${content}${notificationPreferencesDialogMarkup()}`, { tab: 'notifications' });
        startNotificationVisibilityTracking();
    }

    async function renderNotifications() {
        stopNotificationVisibilityTracking();
        const sequence = ++state.notificationListSequence;
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        refreshNativePushRegistration({ requestAuthorization: true });
        const url = '/notifications?history=1&per_page=50';
        const [notificationsResponse, preferencesResponse] = await Promise.all([
            api(url, { expectedIdentityScope: scope }),
            api(notificationPreferencesEndpoint(), { expectedIdentityScope: scope }),
        ]);
        if (sequence !== state.notificationListSequence || scope !== currentUploadIdentityScope()
            || generation !== state.navigationGeneration || state.route !== 'notifications') return;
        state.notificationItems = notificationsResponse.data || [];
        preparePaginatedList('notifications', url, notificationsResponse, state.notificationItems, notificationListMarkup);
    ...
    async function renderAbsences(params = {}) {
        ...
        const query = new URLSearchParams({ per_page: '20', ...(year !== 'all' ? { year } : {}), ...(requestedEmployeeId > 0 ? { employee_id: String(requestedEmployeeId) } : {}) });
        const pendingQuery = new URLSearchParams({ status: 'submitted', approval_scope: '1', per_page: '20', ...(year !== 'all' ? { year } : {}), ...(requestedEmployeeId > 0 ? { employee_id: String(requestedEmployeeId) } : {}) });
        const selectedStatus = String(params.status || 'all');
        if (selectedStatus !== 'all') query.set('status', selectedStatus);
        const splitPending = can('absences.approve') && selectedStatus === 'all';
        if (splitPending) query.set('exclude_pending', '1');
        const [absenceResponse, pendingResponse, contextResponse] = await Promise.all([
            api(`/dossiers/${state.activeDossierId}/absences?${query}`),
            splitPending
                ? api(`/dossiers/${state.activeDossierId}/absences?${pendingQuery}`).catch((error) => ({ data: [], loadError: error }))
        ...
        const pendingList = pendingResponse.loadError ? hrPendingFailure(pendingResponse.loadError, 'absences') : pendingItems.length ? `${mobileValidationSectionTitle(Number(pendingResponse.meta?.pagination?.total) || pendingItems.length)}${hrListMarkup('absences_pending', `/dossiers/${state.activeDossierId}/absences?${pendingQuery}`, pendingResponse, pendingItems, (item) => mobileAbsenceItemMarkup(item, true), true)}` : '';
        ...
        const list = items.length ? `${historyTitle}${hrListMarkup('absences', `/dossiers/${state.activeDossierId}/absences?${query}`, absenceResponse, items, (item) => mobileAbsenceItemMarkup(item, selectedStatus === 'submitted' && canValidate))}` : (pendingItems.length || pendingResponse.loadError ? '' : emptyState('Aucune absence', 'Aucune demande d’absence ne correspond aux filtres sélectionnés.'));
        ...
        state.routeParams = { employeeId, year, ...(selectedStatus !== 'all' ? { status: selectedStatus } : {}), ...(absenceId > 0 ? { absenceId } : {}) };
        persistNavigation();
        renderFrame('Absences', `${actions}${filters}${hrStatusFilterMarkup(selectedStatus)}${balanceMarkup}${pendingList}${list}`, { tab: employeeModuleTab('absences') });
        if (absenceId > 0) window.requestAnimationFrame(async () => {
            ...
            await openHrLinkedDetail('absence', absenceId);
        });
    }
    ...
    async function renderExpenses(params = {}) {
        ...
        const year = String(params.year || currentYear) === 'all' ? 'all' : (/^\d{4}$/.test(String(params.year || '')) ? String(params.year) : currentYear);
        const requestedEmployeeId = Math.max(0, Number(params.employeeId) || 0);
        const query = new URLSearchParams({ status: 'all', per_page: '20', ...(year !== 'all' ? { year } : {}), ...(requestedEmployeeId > 0 ? { employee_id: String(requestedEmployeeId) } : {}) });
        const pendingQuery = new URLSearchParams({ status: 'submitted', approval_scope: '1', per_page: '20', ...(year !== 'all' ? { year } : {}), ...(requestedEmployeeId > 0 ? { employee_id: String(requestedEmployeeId) } : {}) });
        const selectedStatus = String(params.status || 'all');
        if (selectedStatus !== 'all') query.set('status', selectedStatus);
        const splitPending = can('expenses.approve') && selectedStatus === 'all';
        if (splitPending) query.set('exclude_pending', '1');
        const [response, pendingResponse, contextResponse] = await Promise.all([
            api(`/dossiers/${state.activeDossierId}/expense-reports?${query}`),
            splitPending
                ? api(`/dossiers/${state.activeDossierId}/expense-reports?${pendingQuery}`).catch((error) => ({ data: [], loadError: error }))
                : Promise.resolve({ data: [] }),
            api(`/dossiers/${state.activeDossierId}/hr/context?employee_id=${requestedEmployeeId}`),
        ]);
        const context = contextResponse.data || {};
        ...
        const pendingList = pendingResponse.loadError ? hrPendingFailure(pendingResponse.loadError, 'expenses') : pendingItems.length ? `${mobileValidationSectionTitle(Number(pendingResponse.meta?.pagination?.total) || pendingItems.length)}${hrListMarkup('expenses_pending', `/dossiers/${state.activeDossierId}/expense-reports?${pendingQuery}`, pendingResponse, pendingItems, (item) => mobileExpenseItemMarkup(item, true), true)}` : '';
        const historyTitle = pendingItems.length && items.length ? '<div class="mobile-app-section-title mobile-app-validation-history-title"><h2>Historique</h2></div>' : '';
        const list = items.length ? `${historyTitle}${hrListMarkup('expenses', `/dossiers/${state.activeDossierId}/expense-reports?${query}`, response, items, (item) => mobileExpenseItemMarkup(item, selectedStatus === 'submitted' && canValidate))}` : (pendingItems.length || pendingResponse.loadError ? '' : emptyState('Aucune note de frais', 'Aucune note ne correspond à ces filtres.'));
        const reportId = Math.max(0, Number(params.reportId) || 0);
        state.routeParams = { employeeId, year, ...(selectedStatus !== 'all' ? { status: selectedStatus } : {}), ...(reportId > 0 ? { reportId } : {}) };
        persistNavigation();
        renderFrame('Notes de frais', `${actions}${filters}${hrStatusFilterMarkup(selectedStatus)}${pendingList}${list}`, { tab: employeeModuleTab('expenses') });
        if (reportId > 0) window.requestAnimationFrame(async () => {
            const target = app.querySelector(`[data-expense-detail="${reportId}"]`);
            target?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            target?.focus({ preventScroll: true });
            await openHrLinkedDetail('expense', reportId);
        });
    }
    ...
    async function renderExpenseResume(params = {}) {
        const reportId = Number(params.reportId) || 0;
        if (!reportId) return navigate('home');
        const scope = mobileDraftScope();
        const generation = state.navigationGeneration;
        const report = (await api(`/dossiers/${state.activeDossierId}/expense-reports/${reportId}`)).data || {};
        if (scope !== mobileDraftScope() || generation !== state.navigationGeneration) return;
        const items = Array.isArray(report.items) ? report.items : [];
        const editable = Boolean(report.can_update) && !state.readOnly;
        const selected = items.find((item) => Number(item.id) === Number(params.itemId) && item.category !== 'mileage');
        const back = `<button type="button" class="mobile-app-inline-back" data-mobile-resume-back="${selected ? reportId : 0}">← <span>${selected ? 'Reprendre la note' : 'Accueil'}</span></button>`;
        let content;
        if (selected && editable) {
            const receipt = selected.receipt_url
                ? `<button type="button" class="mobile-app-secondary-button" data-open-binary="${escapeHtml(selected.receipt_url)}" data-open-binary-title="${escapeHtml(selected.receipt_filename || 'Justificatif')}">Justificatif joint</button>`
                : `<p class="mobile-app-quick-actions-hint">${escapeHtml(selected.receipt_missing_reason || 'Sans justificatif')}</p>`;
            content = `<form class="mobile-app-form-panel mobile-app-form" data-mobile-form="expense_resume_item">
                <input type="hidden" name="report_id" value="${reportId}"><input type="hidden" name="item_id" value="${Number(selected.id)}"><input type="hidden" name="lock_version" value="${Number(report.lock_version) || 0}">
                <input type="hidden" name="category" value="${escapeHtml(selected.category || 'other')}"><input type="hidden" name="payment_method" value="${escapeHtml(selected.payment_method || 'personal')}"><input type="hidden" name="merchant" value="${escapeHtml(selected.merchant || '')}">
                ${selected.receipt_ged_document_id ? `<input type="hidden" name="receipt_ged_document_id" value="${Number(selected.receipt_ged_document_id)}">` : `<input type="hidden" name="receipt_missing" value="1"><input type="hidden" name="receipt_missing_reason" value="${escapeHtml(selected.receipt_missing_reason || '')}">`}
                <div class="mobile-app-field"><label>Nature de la dépense</label><input name="business_purpose" maxlength="160" required value="${escapeHtml(selected.business_purpose || '')}" data-mobile-quick-entry></div>
                <div class="mobile-app-field"><label>Date des frais</label><input type="date" name="expense_date" required value="${escapeHtml(selected.expense_date || '')}" data-mobile-quick-entry></div>
                <div class="mobile-app-form-row"><div class="mobile-app-field"><label>Montant TTC</label><input type="number" name="amount_including_tax" min="0.01" step="0.01" inputmode="decimal" required value="${escapeHtml(selected.amount_including_tax || '')}" data-mobile-quick-entry></div><div class="mobile-app-field"><label>TVA</label><input type="number" name="vat_amount" min="0" step="0.01" inputmode="decimal" value="${escapeHtml(selected.vat_amount || '')}" data-mobile-quick-entry></div></div>
                <div class="mobile-app-field"><label>Commentaire (facultatif)</label><textarea name="comment" maxlength="1000" data-mobile-quick-entry>${escapeHtml(selected.comment || '')}</textarea></div>
                ${receipt}<p class="mobile-app-form-error" data-mobile-form-error role="alert" hidden></p><button type="submit" class="mobile-app-primary-button">Enregistrer les modifications</button></form>`;
        } else {
            const rows = items.map((item) => `<article class="mobile-app-list-item"><div class="mobile-app-list-item__top"><h3>${escapeHtml(item.business_purpose || report.title || 'Dépense')}</h3>${editable && item.category !== 'mileage' ? `<button type="button" class="mobile-app-quick-actions-edit" data-mobile-resume-item="${Number(item.id)}" data-report-id="${reportId}">Modifier</button>` : ''}</div><p>${formatCivilDate(item.expense_date)} - ${formatMoney(item.amount_including_tax)}</p>${item.receipt_url ? `<button type="button" class="mobile-app-quick-actions-edit" data-open-binary="${escapeHtml(item.receipt_url)}" data-open-binary-title="${escapeHtml(item.receipt_filename || 'Justificatif')}">Justificatif joint</button>` : ''}</article>`).join('');
            content = `<section class="mobile-app-form-panel"><h2>${escapeHtml(report.title || 'Note de frais')}</h2><p>${escapeHtml(report.status_label || humanStatus(report.status))} - ${formatMoney(report.requested_total)}</p>${report.rejection_comment ? `<p>${escapeHtml(report.rejection_comment)}</p>` : ''}</section><div class="mobile-app-list">${rows}</div>
                ${editable && !items.length ? `<button type="button" class="mobile-app-primary-button" data-mobile-resume-complete="${reportId}">Compléter la dépense</button>` : ''}
                ${Boolean(report.can_submit) && items.length && !state.readOnly ? `<form data-mobile-form="expense_resume_submit"><p class="mobile-app-form-error" data-mobile-form-error role="alert" hidden></p><input type="hidden" name="report_id" value="${reportId}"><input type="hidden" name="lock_version" value="${Number(report.lock_version) || 0}"><button type="submit" class="mobile-app-primary-button">Demander le remboursement</button></form>` : ''}`;
        }
        renderFrame(selected && editable ? 'Modifier la dépense' : 'Reprendre la note', back + content, { tab: employeeModuleTab('expenses') });
    }
    ...
    async function renderExpenseItemForm(params = {}) {
        ...
        const report = response.data || {};
        if (scope !== mobileDraftScope() || generation !== state.navigationGeneration) return;
        if ((report.items || []).length || report.can_update === false) return navigate('expense_resume', { reportId }, false);
    ...
    async function renderMileage(params = {}) {
        ...
        const [configResponse, tripsResponse] = await Promise.all([
            ...
            api(`/dossiers/${state.activeDossierId}/mileage/trips?${new URLSearchParams({ period, month, year, status, per_page: '50' })}`),
        ]);
        if (state.route !== 'mileage' || requestedScope !== mobileDataScope() || navigationGeneration !== state.navigationGeneration) return false;
        ...
        if (!availableValues.has(periodValue) && period !== 'all') {
            if (!availableByYear.has(year)) availableByYear.set(year, new Set());
            if (period === 'month') {
                const selectedYear = month.slice(0, 4);
                if (!availableByYear.has(selectedYear)) availableByYear.set(selectedYear, new Set());
                availableByYear.get(selectedYear).add(month);
            }
        }
    ...
    async function renderMileageTripForm(params = {}) {
        ...
        if (Number(params.id) > 0) {
            const response = await api(`/dossiers/${state.activeDossierId}/mileage/trips/${Number(params.id)}`);
    ...
    async function renderBilling(params = {}) {
        if (!can('billing.read')) {
            renderFrame('Facturation', emptyState('Consultation non autorisée', 'Le module Facturation n’est pas accessible pour ce dossier.'), { tab: 'billing' });
            return;
        }
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const view = ['receivables', 'quotes', 'overdue'].includes(params.view) ? params.view : 'receivables';
        const page = Math.max(1, Number.parseInt(params.page, 10) || 1);
        const documentId = Math.max(0, Number.parseInt(params.documentId, 10) || 0);
        const base = `/dossiers/${state.activeDossierId}/billing/documents`;
        let response;
        try {
            response = await api(documentId ? `${base}/${documentId}` : `${base}?view=${view}&page=${page}`, { offlineCache: false });
        } catch (error) {
            if (scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration || state.route !== 'billing') return;
            const message = state.offline || error.code === 'offline_unavailable'
                ? 'Hors ligne - reconnectez-vous pour actualiser la facturation.'
                : (error.message || 'La facturation ne peut pas être chargée.');
            renderFrame('Facturation', `<div class="mobile-app-form-panel"><p role="status">${escapeHtml(message)}</p><button type="button" class="mobile-app-secondary-button" data-billing-refresh>Réessayer</button></div>`, { tab: 'billing' });
            app.querySelector('[data-billing-refresh]')?.addEventListener('click', () => navigate('billing', params, false));
            return;
        }
        if (scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration || state.route !== 'billing') return;
        const data = response.data;
        if (!data || (!documentId && !Array.isArray(data.documents))) throw new Error('La réponse de facturation est incomplète.');
        const refreshed = data.refreshed_at_utc ? mobileInstantLabel(data.refreshed_at_utc) : 'Date indisponible';
        const toolbar = `<div class="mobile-app-section-title mobile-app-billing-toolbar"><span>Actualisé le ${escapeHtml(refreshed)}</span><button type="button" class="mobile-app-secondary-button" data-billing-refresh>Actualiser</button></div>`;
        let content;
        if (documentId) {
            const rows = [['Document', `${data.type_label} ${data.number}`], ['Client', data.client], ['État', data.status_label], ['Date', data.issue_date ? formatCivilDate(data.issue_date) : 'Non renseignée'], ['Échéance', data.due_date ? formatCivilDate(data.due_date) : 'Non renseignée'], ['Total HT', data.subtotal_ht_label], ['TVA', data.vat_total_label], ['Total TTC', data.total_label]];
            if (data.type !== 'quote') rows.push(['Réglé', data.paid_total_label], ['Reste à encaisser', data.remaining_label]);
            content = `<button type="button" class="mobile-app-secondary-button" data-billing-back>Retour à la liste</button><article class="mobile-app-form-panel mobile-app-billing-detail">${data.overdue ? '<p class="mobile-app-billing-overdue">Échéance dépassée</p>' : ''}${rows.map(([label, value]) => `<div><small>${escapeHtml(label)}</small><strong>${escapeHtml(value)}</strong></div>`).join('')}${data.pdf_available && data.content_url ? `<button type="button" class="mobile-app-primary-button" data-open-binary="${escapeHtml(data.content_url)}">Ouvrir le PDF original</button>` : '<p>Le PDF original n’est pas disponible.</p>'}</article>`;
        } else {
            const counts = data.counts || {};
            const options = [['receivables', 'À encaisser'], ['quotes', 'Devis en attente'], ['overdue', 'En retard']];
            const filters = `<div class="mobile-app-billing-filters">${options.map(([key, label]) => `<button type="button" class="mobile-app-billing-filter" data-billing-view="${key}" aria-pressed="${view === key}">${label} - ${Number(counts[key]) || 0}</button>`).join('')}</div>`;
            const list = data.documents.length ? `<div class="mobile-app-list">${data.documents.map(item => `<button type="button" class="mobile-app-list-item mobile-app-list-button mobile-app-billing-item" data-billing-document="${Number(item.id)}"><h3>${escapeHtml(item.type_label)} ${escapeHtml(item.number)}</h3><p>${escapeHtml(item.client)}</p><p>${escapeHtml(item.amount_caption)} - ${escapeHtml(item.amount_label)}</p><p>${item.due_date ? `Échéance ${escapeHtml(formatCivilDate(item.due_date))}` : 'Échéance non renseignée'}${item.overdue ? ' - En retard' : ''}</p></button>`).join('')}</div>` : emptyState('Facturation', data.empty_message || 'La liste ne peut pas être déterminée.');
            const pagination = response.meta?.pagination || {};
            const controls = `<div class="mobile-app-billing-pagination">${page > 1 ? `<button type="button" class="mobile-app-secondary-button" data-billing-page="1">Première page</button><button type="button" class="mobile-app-secondary-button" data-billing-page="${page - 1}">Précédent</button>` : ''}${pagination.has_more ? `<button type="button" class="mobile-app-secondary-button" data-billing-page="${page + 1}">Suivant</button>` : ''}</div>`;
            content = `${filters}${list}${controls}`;
        }
        renderFrame('Facturation', `${toolbar}${content}`, { tab: 'billing' });
        app.querySelector('[data-billing-refresh]')?.addEventListener('click', () => navigate('billing', params, false));
        app.querySelector('[data-billing-back]')?.addEventListener('click', () => navigate('billing', { view, page }, false));
        app.querySelectorAll('[data-billing-view]').forEach(button => button.addEventListener('click', () => navigate('billing', { view: button.dataset.billingView }, false)));
        app.querySelectorAll('[data-billing-page]').forEach(button => button.addEventListener('click', () => navigate('billing', { view, page: Number(button.dataset.billingPage) }, false)));
        app.querySelectorAll('[data-billing-document]').forEach(button => button.addEventListener('click', () => navigate('billing', { view, page, documentId: Number(button.dataset.billingDocument) })));
    }
    ...
    function syncMobileReportFields(form) {
        ...
        form.querySelectorAll('[data-report-period-preset]').forEach((button) => {
            ...
            const ranges = JSON.parse(form.dataset.periods || '{}');
            const range = ranges[button.dataset.reportPeriodPreset];
            const selected = !snapshot && range && range.start === form.elements.period_start.value && range.end === form.elements.period_end.value;
            button.classList.toggle('is-selected', Boolean(selected));
            button.setAttribute('aria-pressed', String(Boolean(selected)));
            button.textContent = snapshot ? 'Aujourd’hui' : ({ all: 'Période complète', year: 'Année en cours', month: 'Mois en cours', current: 'Exercice courant', previous: 'Exercice précédent' }[button.dataset.reportPeriodPreset]);
        });
        const summary = form.querySelector('[data-report-period-summary]');
        if (summary) summary.textContent = snapshot ? `En date du ${formatCivilDate(form.elements.as_of.value)}` : `Du ${formatCivilDate(form.elements.period_start.value)} au ${formatCivilDate(form.elements.period_end.value)}`;
        syncMobileReportSelectionActions(form);
    }

    function mobileReportDescription(type) {
        return ({ sig: 'Les soldes intermédiaires de gestion calculés à partir des écritures de la période.', general_ledger: 'Le détail des écritures, compte par compte.', journal: 'Les écritures regroupées par journal comptable.', immobilisations: 'La situation des immobilisations à la date choisie.', loans: 'La situation des emprunts à la date choisie.' })[type] || 'Une vue des mouvements et soldes de vos comptes.';
    }
    ...
    function mobileReportHistoryMarkup(reports) {
        ...
        return `<div class="mobile-app-report-history">${reports.map((report) => {
            const ready = report.status === 'ready' && report.content_url && (!report.expires_at_utc || Date.parse(report.expires_at_utc) > Date.now());
            ...
            return `<article class="mobile-app-report-history__item ${ready ? 'is-ready' : ''}">
                <button type="button" class="mobile-app-report-history__open" ${ready ? `data-open-binary="${escapeHtml(report.content_url)}"` : 'disabled'} aria-label="Ouvrir ${escapeHtml(name)}">
                    ...
                    <span class="mobile-app-report-history__copy"><strong title="${escapeHtml(name)}">${escapeHtml(name)}</strong><time datetime="${escapeHtml(report.created_at_utc || '')}">${escapeHtml(generatedAt)}</time><small class="mobile-app-report-detail" role="status">${escapeHtml(report.stage_label || humanStatus(report.status))}${ready && report.expires_at_utc ? ` - jusqu’au ${escapeHtml(formatDate(report.expires_at_utc, true))}` : ''}</small>${report.error?.message ? `<small class="mobile-app-report-detail is-error" role="alert">${escapeHtml(report.error.message)}</small>` : ''}</span>
                </button>
                ${processing ? `<span class="mobile-app-report-history__spinner" role="status" aria-label="Génération de ${escapeHtml(name)} en cours"></span>` : ''}
                <span class="mobile-app-report-history__actions">
                    ${!ready && !processing && report.can_regenerate && can('reports.generate') ? `<button type="button" class="mobile-app-report-history__action" data-report-regenerate="${Number(report.id)}" aria-label="Régénérer ${escapeHtml(name)}" title="Régénérer">↻</button>` : ''}
    ...
    function scheduleMobileReportRefresh(reports) {
        ...
        const active = reports.filter((report) => ['queued', 'running'].includes(String(report.status || '')));
        if (!active.length || state.route !== 'reports') return;
        const scope = mobileDataScope();
        const generation = state.navigationGeneration;
        const dossier = state.activeDossierId;
        state.reportRefreshTimer = window.setTimeout(async () => {
            state.reportRefreshTimer = null;
            if (state.route !== 'reports' || scope !== mobileDataScope() || generation !== state.navigationGeneration) return;
            if (state.reportRefreshInFlight || document.hidden || state.applicationBackgrounded || state.biometricLocked
                || (iosNativeRuntime && window.__MAISON_PILOTE_IOS_CONTENT_VISIBLE__ !== true)) {
                scheduleMobileReportRefresh(reports); return;
            }
            state.reportRefreshInFlight = true;
            let next = reports;
            try {
                const query = new URLSearchParams({ per_page: '50' });
                active.slice(0, 50).forEach((report) => query.append('ids[]', String(report.id)));
                const response = await api(`/dossiers/${dossier}/reports?${query}`, { expectedIdentityScope: scope });
                if (scope !== mobileDataScope() || generation !== state.navigationGeneration || state.route !== 'reports') return;
                const updates = new Map((response.data || []).map((report) => [Number(report.id), report]));
                const list = state.paginatedLists.reports;
                if (!list) return;
                list.items = list.items.map((report) => updates.get(Number(report.id)) || report);
                next = list.items;
                if (state.routeParams?.status === 'processing' && next.some((report) => !['queued', 'running'].includes(report.status))) {
                    await renderReportForm(); next = state.paginatedLists.reports?.items || []; return;
                }
                const node = app.querySelector('[data-mobile-paginated-list="reports"]');
                if (node) node.outerHTML = paginationMarkup('reports');
                const error = app.querySelector('[data-report-refresh-error]');
                if (error) error.hidden = true;
            } catch (error) {
                const target = app.querySelector('[data-report-refresh-error]');
                if (target) { target.hidden = false; target.querySelector('span').textContent = 'Actualisation interrompue. Les derniers états reçus restent affichés.'; }
            } finally {
                state.reportRefreshInFlight = false;
                if (scope === mobileDataScope() && generation === state.navigationGeneration) scheduleMobileReportRefresh(next);
            }
        }, 3000);
    }
    ...
    function mobileReportCreationForm(data, dossierId) {
        ...
        return `<form class="mobile-app-form-panel mobile-app-report-form" data-mobile-form="report" data-dossier-id="${dossierId}" data-default-start="${escapeHtml(periodStart)}" data-default-end="${escapeHtml(periodEnd)}" data-periods="${escapeHtml(JSON.stringify({ all: data.full_period || data.default_period, current: data.current_exercise, previous: data.previous_exercise }))}"><div class="mobile-app-report-form__fields"><p class="mobile-app-form-hint">${escapeHtml(activeDossier()?.name || '')}</p>${typeChoices}<p class="mobile-app-form-hint" data-report-description></p><div class="mobile-app-field"><label>Format</label><div class="mobile-app-report-types">${['pdf', 'xlsx'].map((format) => `<button type="button" data-mobile-report-format-choice="${format}" class="${format === 'pdf' ? 'is-selected' : ''}" aria-pressed="${format === 'pdf'}">${format === 'pdf' ? 'PDF' : 'Excel'}</button>`).join('')}</div><input type="hidden" name="format" value="pdf"></div><div class="mobile-app-field"><label data-report-period-label>Période</label><div class="mobile-app-report-types">${[...(data.current_exercise ? ['current'] : []), ...(data.previous_exercise ? ['previous'] : []), 'all', 'year', 'month'].map((preset) => `<button type="button" data-report-period-preset="${preset}"></button>`).join('')}</div></div>${periodFields}<p class="mobile-app-form-hint" data-report-period-summary></p>${filters}${supplementalFields}</div><div class="mobile-app-report-form__footer"><p class="mobile-app-form-error" data-mobile-form-error role="alert" hidden></p><p class="mobile-app-form-hint">La génération se poursuit en arrière-plan. Vous serez averti dès que l’état sera prêt.</p>${writeButton('Lancer la génération')}</div></form>`;
    }

    async function renderReportForm(message = '') {
        const scope = mobileDataScope();
        const filter = ['processing', 'ready', 'failed', 'expired'].includes(state.routeParams?.status) ? state.routeParams.status : 'all';
        const url = `/dossiers/${state.activeDossierId}/reports?per_page=20&status=${filter}`;
        const response = await api(url, { expectedIdentityScope: scope });
        if (scope !== mobileDataScope() || state.route !== 'reports') return;
        const history = Array.isArray(response.data) ? response.data : [];
        const button = can('reports.generate') ? `<button class="mobile-app-primary-button mobile-app-report-create-toggle" type="button" data-report-create-toggle><span>Générer un nouvel état</span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 5h2v6h6v2h-6v6h-2v-6H5v-2h6V5Z"/></svg></button>` : '';
        const filters = `<div class="mobile-app-field mt-3"><label>États générés</label><select data-report-history-filter>${[['all', 'Tous'], ['processing', 'En cours'], ['ready', 'Disponibles'], ['failed', 'En erreur'], ['expired', 'Expirés']].map(([value, label]) => `<option value="${value}" ${filter === value ? 'selected' : ''}>${label}</option>`).join('')}</select></div>`;
        const list = preparePaginatedList('reports', url, response, history, (items) => items.length ? mobileReportHistoryMarkup(items)
            : emptyState(filter === 'all' ? 'Aucun état généré' : 'Aucun état pour ce filtre', filter === 'all' ? 'Les états générés pour ce dossier apparaîtront ici.' : 'Choisissez Tous pour retrouver l’ensemble des états.'));
        renderFrame('États comptables', `${message ? `<div class="mobile-app-settings-message is-success" role="status">${escapeHtml(message)}</div>` : ''}${button}${filters}<div class="mobile-app-settings-message is-error" data-report-refresh-error role="alert" hidden><span></span><button class="mobile-app-quick-actions-edit" type="button" data-report-refresh-retry>Réessayer</button></div>${list}`, { tab: 'reports' });
    ...
    async function openQuickReportCreation() {
        ...
        try {
            const response = await api(`/dossiers/${dossierId}/reports/catalog?history=0`);
    ...
    const secureMobileDrafts = new Map();
    const touchedSecureDraftKeys = new Set();

    function writeSecureMobileDraft(key, value) {
        touchedSecureDraftKeys.add(key);
        if (value !== null) secureMobileDrafts.set(key, String(value));
        else secureMobileDrafts.delete(key);
        if (value !== null) window.MaisonPiloteNative?.secureDrafts?.write?.(key, String(value));
        else window.MaisonPiloteNative?.secureDrafts?.remove?.(key);
    }

    function restoreSecureNoteDrafts() {
        for (const scope of ['personal', 'dossier', 'ai_dossier', 'admin']) {
            const key = `note:${mobileDraftScope()}:${scope}`;
            if (!Object.prototype.hasOwnProperty.call(state.notesDrafts, scope) && secureMobileDrafts.has(key)) {
                state.notesDrafts[scope] = secureMobileDrafts.get(key);
                const field = app.querySelector(`[data-mobile-notes-body="${scope}"]`);
                if (field && field !== document.activeElement) field.value = state.notesDrafts[scope];
            }
        }
    }

    function restoreSecureHomeDraft() {
        const scope = state.homeQuickActionScope;
        if (!scope || !state.homeQuickActionsEditable || state.homeQuickActionDrafts[scope]) return false;
        try {
            const draft = JSON.parse(secureMobileDrafts.get(`home:${scope}`) || 'null');
            if (!Array.isArray(draft?.keys) || !draft.keys.every((key) => typeof key === 'string')) return false;
            state.homeQuickActionDrafts[scope] = {
                keys: [...new Set(draft.keys)].filter((key) => state.homeQuickActionCatalog.some((action) => String(action.key) === key)),
            };
            return true;
        } catch (_) {
            return false;
    ...
    window.addEventListener('maisonpilote:secure-drafts', (event) => {
        const ownerId = Number(state.bootstrap?.profile?.owner_user_id || state.bootstrap?.profile?.id) || 0;
        if (!state.authenticated || !ownerId || Number(event.detail?.identity) !== ownerId) return;
        Object.entries(event.detail?.drafts || {}).forEach(([key, value]) => {
            if (touchedSecureDraftKeys.has(key) || typeof value !== 'string') return;
            secureMobileDrafts.set(key, value);
            if (key.startsWith(`message:${ownerId}:`)) state.messageDrafts[key] = value;
        });
        restoreSecureNoteDrafts();
        if (state.route === 'home' && !state.quickActionsEditing && restoreSecureHomeDraft()) {
            state.homeQuickActionKeys = [...state.homeQuickActionDrafts[state.homeQuickActionScope].keys];
            state.quickActionsEditing = true;
            renderQuickActionsHome();
        }
        const composer = app.querySelector('#mobile-message-body');
        if (composer && composer !== document.activeElement) restoreMessageDraft();
    });
    window.addEventListener('maisonpilote:secure-drafts-error', (event) => {
        if (String(event.detail?.key || '').startsWith('note:')) setMobileNotesStatus('Sauvegarde locale impossible. Gardez l’application ouverte jusqu’à la synchronisation.');
        else showQuickCreationNotice(event.detail?.message || 'Sauvegarde locale du brouillon impossible.');
    });

    function mobileDraftScope() {
        const profile = state.bootstrap?.profile || {};
        const ownerId = Number(profile.owner_user_id || profile.id) || 0;
        return `${ownerId}:${Number(profile.id) || ownerId}:${Number(state.activeDossierId) || 0}`;
    }

    function messageDraftKey(conversationId = state.messageConversationId) {
        return `message:${mobileDraftScope()}:${Number(conversationId) || 0}`;
    }

    function rememberMessageDraft() {
        const composer = app.querySelector('#mobile-message-body');
        if (!(composer instanceof HTMLTextAreaElement)) return;
        const key = composer.dataset.draftKey || messageDraftKey();
        state.messageDrafts[key] = composer.value;
        writeSecureMobileDraft(key, composer.value);
    }

    function restoreMessageDraft() {
        const composer = app.querySelector('#mobile-message-body');
        if (!(composer instanceof HTMLTextAreaElement)) return;
        composer.dataset.draftKey = messageDraftKey();
        composer.value = state.messageDrafts[messageDraftKey()] || '';
    }
        ...
        const messageListMarkup = messages.length
            ? messages.flatMap((message) => {
                ...
                const attachmentMarkup = attachments.length
                    ? `<div class="mobile-app-message__attachments">${attachments.map((attachment) => `<button class="mobile-app-message-attachment" type="button" data-mobile-message-attachment="${escapeHtml(attachment.content_url || attachment.url || '')}" data-filename="${escapeHtml(attachment.name || 'Pièce jointe')}" aria-label="Ouvrir ${escapeHtml(attachment.name || 'la pièce jointe')}">${quickActionIcons.file}<span>${escapeHtml(attachment.name || 'Pièce jointe')}</span></button>`).join('')}</div>`
                    : '';
    ...
    function scrollMessageConversationToBottom() {
        const indicator = app.querySelector('[data-mobile-message-new]');
    ...
    function updateMessageConversation(data, forceScroll = false) {
        const previousLastId = state.messagePayload?.messages?.at(-1)?.id;
        const previousConversationId = state.messageConversationId;
        ...
        if (previousConversationId !== presentation.conversationId) restoreMessageDraft();
        persistNavigation();
        ...
        if (presentation.signature !== state.messageListSignature) {
            ...
            else if (data.messages?.at(-1)?.id !== previousLastId) {
                const indicator = app.querySelector('[data-mobile-message-new]');
    ...
    async function renderMessages(params = {}) {
        ...
        const response = await api(messageConversationEndpoint());
        if (state.route !== 'messages' || requestedScope !== mobileDataScope() || navigationGeneration !== state.navigationGeneration) return false;
        const data = response.data || {};
        const presentation = messagePresentation(data);
        state.messagePayload = data;
        state.messageConversationId = presentation.conversationId;
        ...
        renderFrame('Messagerie', `<div class="mobile-app-message-thread">
            ...
            <div class="mobile-app-message-list">${presentation.messageListMarkup}</div><button class="mobile-app-primary-button mobile-app-new-messages" type="button" data-mobile-message-new hidden><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m20 12-1.41-1.41L13 16.17V4h-2v12.17l-5.59-5.59L4 12l8 8 8-8Z"/></svg><span>Nouveaux messages</span></button>
        </div>`, {
            tab: 'more',
    ...
    async function renderUserSettings(message = '', messageType = 'success') {
        const scope = currentUploadIdentityScope();
        const generation = state.navigationGeneration;
        const response = await api('/user-settings', { expectedIdentityScope: scope });
        if (scope !== currentUploadIdentityScope() || generation !== state.navigationGeneration || state.route !== 'user_settings') return;
        const data = response.data || {};
        state.accountTimezone = String(data.timezone || '');
        ...
        renderFrame('Paramètres utilisateur', `${alert}${!data.editable ? '<div class="mobile-app-settings-message is-warning">Les paramètres de la personne émulée restent protégés.</div>' : ''}
            ...
            ${biometricSettings}
            ${!disabled && data.two_factor ? '<div data-mobile-two-factor-settings></div>' : ''}
        ...
        const factorPanel = app.querySelector('[data-mobile-two-factor-settings]');
        const identityScope = currentOfflineIdentityScope();
        if (factorPanel && mobileFactorSettings?.suspendedAt && identityScope
            && mobileFactorSettings.identityScope === identityScope
            && Date.now() - mobileFactorSettings.suspendedAt < 300000 && !state.biometricLocked) {
            factorPanel.replaceWith(mobileFactorSettings.panel);
            mobileFactorSettings.suspendedAt = 0;
            clearTimeout(mobileFactorExpiryTimer);
        } else {
            forgetMobileFactorSettings();
            if (factorPanel) mobileFactorSettings = {
                panel: factorPanel, identityScope, suspendedAt: 0,
                dispose: mountTwoFactorSettings(factorPanel, {
                    initial: data.two_factor, mobile: true,
                    request: (body) => api('/user-settings/two-factor', { method: body ? 'POST' : 'GET', ...(body ? { body } : {}) }),
                    openAuthenticator: iosNativeRuntime ? (uri) => {
                        if (!window.MaisonPiloteNative?.supportsAuthenticatorLinks) {
                            throw new Error('Copiez la clé affichée dans votre application d’authentification. L’ouverture directe est disponible à partir de Maison Pilote 1.133.');
                        }
                        window.MaisonPiloteNative.openExternal(uri);
                    } : null,
                }),
            };
        }
    ...
    function forgetMobileFactorSettings() {
        clearTimeout(mobileFactorExpiryTimer);
        mobileFactorExpiryTimer = null;
        mobileFactorSettings?.dispose?.();
        mobileFactorSettings = null;
    }
    ...
    async function submitMobileForm(form) {
        ...
        if (['expense_resume_item', 'expense_resume_submit'].includes(kind)) {
            const reportId = Number(data.report_id) || 0;
            const itemId = Number(data.item_id) || 0;
            if (!reportId || (kind === 'expense_resume_item' && !itemId)) return;
            const scope = mobileDraftScope();
            const dossierId = state.activeDossierId;
            const generation = state.navigationGeneration;
            delete data.report_id;
            delete data.item_id;
            const submit = kind === 'expense_resume_submit';
            if (!submit) {
                const vat = String(form.elements.namedItem('vat_amount')?.value || '').trim();
                data.vat_amount = vat === '' ? null : vat;
                data.comment = String(form.elements.namedItem('comment')?.value || '');
            }
            const body = submit ? { transition: 'submit', lock_version: Number(data.lock_version) || 0 } : data;
            creationFormError(form);
            setCreationFormBusy(form, true, submit ? 'Envoi…' : 'Enregistrement…');
            try {
                await api(`/dossiers/${dossierId}/expense-reports/${reportId}/${submit ? 'transition' : `items/${itemId}`}`, {
                    method: submit ? 'POST' : 'PATCH', body, idempotencyKey: creationIdempotencyKey(form, body),
                });
                if (scope === mobileDraftScope() && generation === state.navigationGeneration && form.isConnected && state.route === 'expense_resume') {
                    await navigate(submit ? 'home' : 'expense_resume', submit ? {} : { reportId }, false);
                }
            } catch (error) {
                creationFormError(form, error?.message || 'La note n’a pas pu être enregistrée.');
                setCreationFormBusy(form, false, submit ? 'Demander le remboursement' : 'Enregistrer les modifications');
            }
            return;
        }
        ...
        if (kind === 'expense_item') {
            ...
            try {
                const headerResponse = await api(`/dossiers/${dossierId}/expense-reports/${reportId}`, {
                    method: 'PATCH',
                    body: {
                ...
                const response = await api(`/dossiers/${dossierId}/expense-reports/${reportId}/items`, { method: 'POST', body: data });
                if (scope !== mobileDraftScope() || generation !== state.navigationGeneration) return;
                await api(`/dossiers/${dossierId}/expense-reports/${reportId}/transition`, {
                    method: 'POST',
                ...
                if (scope === mobileDraftScope() && generation === state.navigationGeneration) renderError(error, state.route);
            }
            return;
        }
    ...
    app.addEventListener('click', (event) => {
        const attachment = event.target.closest('[data-mobile-message-attachment]');
        if (attachment) {
            event.preventDefault(); event.stopImmediatePropagation();
            void openBinary(attachment.dataset.mobileMessageAttachment, attachment);
        }
        if (event.target.closest('[data-mobile-message-new]')) {
            event.preventDefault(); event.stopImmediatePropagation(); scrollMessageConversationToBottom();
        }
    }, true);
    app.addEventListener('scroll', (event) => {
        const content = event.target;
        if (content.matches?.('.mobile-app-content--messages') && content.scrollHeight - content.scrollTop - content.clientHeight <= 80) {
            const indicator = app.querySelector('[data-mobile-message-new]');
            if (indicator) indicator.hidden = true;
        }
    }, true);
    ...
    app.addEventListener('pointerdown', (event) => {
        if (event.button !== 0 || state.route !== 'documents') return;
        ...
        if (!item || item.dataset.mobileDocumentCanSelect !== '1') return;
    ...
    app.addEventListener('contextmenu', (event) => {
        if (event.target.closest('[data-mobile-home-news-link]')) return;
        ...
        if (state.route === 'documents' && documentItem?.dataset.mobileDocumentCanSelect === '1') {
            event.preventDefault();
    ...
    app.addEventListener('click', async (event) => {
        const privacyLink = event.target.closest('a.mobile-app-privacy-link');
        if (privacyLink) {
            event.preventDefault();
            const url = 'https://maisonpilote.fr/confidentialite';
            if (iosNativeRuntime && window.MaisonPiloteNative?.openExternal) window.MaisonPiloteNative.openExternal(url);
            else window.open(url, '_blank', 'noopener,noreferrer');
            return;
        }
        if (event.target.closest('[data-mobile-home-preferences-retry]')) {
            event.preventDefault();
            await requestHomePreferences(state.homePreferencesRetryValue);
            return;
        }
        const newsLink = event.target.closest('[data-mobile-home-news-link]');
        if (newsLink) {
            event.preventDefault();
            const url = homeNewsLink(newsLink.getAttribute('href'));
            if (url) {
                if (iosNativeRuntime && window.MaisonPiloteNative?.openExternal) window.MaisonPiloteNative.openExternal(url);
                else window.open(url, '_blank', 'noopener,noreferrer');
            }
        ...
        if (event.target.closest('[data-mobile-home-news-refresh]')) {
            event.preventDefault();
            await refreshHomeNews();
            return;
        }
        if (event.target.closest('[data-mobile-share-intake-close]')) { app.querySelector('[data-mobile-share-intake-result]')?.remove(); return; }
        const loadMore = event.target.closest('[data-mobile-load-more]');
        if (loadMore) { event.preventDefault(); await loadMoreMobileItems(loadMore.dataset.mobileLoadMore, loadMore); return; }
        ...
        if (state.documentSelectionMode && selectedDocumentItem) {
            ...
            if (selectedDocumentItem.dataset.mobileDocumentCanSelect === '1' && key) {
                if (state.selectedDocumentItems.has(key)) state.selectedDocumentItems.delete(key);
                else state.selectedDocumentItems.add(key);
                syncDocumentSelectionUi();
            }
        ...
        if (event.target.closest('[data-mobile-header-back]')) { await navigateBack(); return; }
        if (event.target.closest('[data-mobile-update-check]')) { await checkForUpdates(true); return; }
        if (event.target.closest('[data-mobile-footer-reset]')) {
            if (state.readOnly || !state.footerNavigationEditable || state.footerNavigationSaving) return;
            state.footerNavigationDraftKeys = defaultFooterRoutes();
            state.footerNavigationMessage = 'Réglages par défaut prêts à enregistrer.';
            rememberFooterDraft();
            renderFooterNavigationSettings();
            return;
        }
        ...
        if (footerMove) {
            if (state.readOnly || !state.footerNavigationEditable || state.footerNavigationSaving || footerMove.disabled) return;
            ...
            if (index > 0 && targetIndex > 0 && targetIndex < state.footerNavigationDraftKeys.length - 1) {
        ...
        if (footerToggle) {
            if (state.readOnly || !state.footerNavigationEditable || state.footerNavigationSaving || footerToggle.disabled) return;
            const route = String(footerToggle.dataset.mobileFooterToggle || '');
            if (!route || ['home', 'more'].includes(route) || !mobileNavigationCatalog().some(([key]) => key === route)) return;
            const selected = state.footerNavigationDraftKeys.includes(route);
            if (!selected && state.footerNavigationDraftKeys.length >= 5) return;
            state.footerNavigationDraftKeys = selected
                ? state.footerNavigationDraftKeys.filter((candidate) => candidate !== route)
                : limitedFooterRoutes([...state.footerNavigationDraftKeys.filter((key) => key !== 'more'), route, 'more']);
            rememberFooterDraft();
            renderFooterNavigationSettings();
            return;
        }
        if (event.target.closest('[data-mobile-footer-save]')) { await saveFooterNavigation(); return; }
        ...
        const mobileCard = event.target.closest('[data-mobile-card-interaction], [data-notification-open]');
        if (mobileCard) {
            const readingOperation = {};
            state.notificationOpenOperation = readingOperation;
            const notificationId = Number(mobileCard.dataset.notificationOpen) || 0;
            const readingScope = currentUploadIdentityScope();
            const readingGeneration = state.navigationGeneration;
            if (notificationId > 0 && !state.readOnly) {
                try {
                    await api(`/notifications/${notificationId}/read`, { method: 'POST', expectedIdentityScope: readingScope });
                    void refreshNotificationBadge(true);
                } catch (_) { /* A failed acknowledgement never optimistically clears the unread count. */ }
                if (readingScope !== currentUploadIdentityScope() || readingGeneration !== state.navigationGeneration
                    || state.notificationOpenOperation !== readingOperation) return;
            }
            try {
        ...
        if (event.target.closest('[data-document-tools-close]')) { if (!state.documentSelectionBusy) app.querySelector('[data-document-tools-dialog]')?.remove(); return; }
        if (event.target.closest('[data-document-filters]')) { openDocumentFilters(); return; }
        if (event.target.closest('[data-document-filters-apply], [data-document-filters-reset]')) {
            const reset = Boolean(event.target.closest('[data-document-filters-reset]'));
            const filters = Object.fromEntries(['type', 'from', 'to'].map((key) => [key, reset ? '' : (app.querySelector(`[data-document-filter-${key}]`)?.value || '')]));
            if (filters.from && filters.to && filters.from > filters.to) {
                const error = app.querySelector('[data-document-tools-error]');
                error.textContent = 'La date de fin doit suivre la date de début.'; error.hidden = false; return;
            }
            closeDocumentSelectionMode();
            await navigate('documents', { ...(state.routeParams || {}), ...filters }, false);
            return;
        }
        const selectionShare = event.target.closest('[data-document-selection-share]');
        if (selectionShare && !selectionShare.disabled) { await applyDocumentSelection('share', selectionShare); return; }
        if (event.target.closest('[data-document-selection-move]')) {
            try { await openDocumentMove(); } catch (error) { showQuickCreationNotice(error?.message || 'Dossiers indisponibles.', 6000); }
            return;
        }
        const moveConfirm = event.target.closest('[data-document-move-confirm]');
        if (moveConfirm && !moveConfirm.disabled) { await applyDocumentSelection('move', moveConfirm); return; }
        ...
        if (documentScopeToggle) {
            ...
            const currentScope = ['all', 'personal', 'recent'].includes(form?.dataset.documentSearchScope)
                ? form.dataset.documentSearchScope
                : 'current';
            const scope = ({ current: 'all', all: 'personal', personal: 'recent', recent: 'current' })[currentScope];
        ...
        if (userMenuToggle) {
            ...
            const cameleonMenu = app.querySelector('[data-mobile-cameleon-menu]');
            if (cameleonMenu) cameleonMenu.hidden = true;
            app.querySelector('[data-mobile-cameleon-menu-toggle]')?.setAttribute('aria-expanded', 'false');
            state.cameleonRequestSequence += 1;
            if (opening) window.requestAnimationFrame(() => { fitHeaderMenus(); menu?.querySelector('button')?.focus({ preventScroll: true }); });
        ...
        if (cameleonMenuToggle) {
            ...
            if (opening) {
                ...
                window.setTimeout(() => { fitHeaderMenus(); input?.focus(); }, 0);
            } else {
                state.cameleonRequestSequence += 1;
            }
            return;
        ...
        if (dossierMenuToggle) {
            ...
            app.querySelector('[data-mobile-cameleon-menu-toggle]')?.setAttribute('aria-expanded', 'false');
            if (opening) window.setTimeout(() => { fitHeaderMenus(); app.querySelector('[data-mobile-dossier-search]')?.focus(); }, 0);
            return;
        }
        ...
        const resumeBack = event.target.closest('[data-mobile-resume-back]');
        if (resumeBack) {
            const reportId = Number(resumeBack.dataset.mobileResumeBack) || 0;
            await navigate(reportId ? 'expense_resume' : 'home', reportId ? { reportId } : {}, false);
            return;
        }
        const resumeItem = event.target.closest('[data-mobile-resume-item]');
        if (resumeItem) {
            await navigate('expense_resume', { reportId: Number(resumeItem.dataset.reportId), itemId: Number(resumeItem.dataset.mobileResumeItem) });
            return;
        }
        const resumeComplete = event.target.closest('[data-mobile-resume-complete]');
        if (resumeComplete) {
            await navigate('expense_item_create', { reportId: Number(resumeComplete.dataset.mobileResumeComplete), step: 'receipt' });
            return;
        }
        const homeRefresh = event.target.closest('[data-mobile-home-refresh]');
        if (homeRefresh) {
            homeRefresh.disabled = true;
            try { await loadBootstrap(); await navigate('home', {}, false); }
            catch (error) { renderError(error, 'home'); }
            return;
        }
        const homeDestination = event.target.closest('[data-mobile-home-destination]');
        if (homeDestination) {
            const destination = String(homeDestination.dataset.mobileHomeDestination || '');
            const expense = destination.match(/^expenses\/(\d+)$/);
            if (expense) await navigate('expense_resume', { reportId: Number(expense[1]) });
            else if (['tasks', 'absences', 'expenses'].includes(destination)) await navigate(destination);
            return;
        }
        if (event.target.closest('[data-quick-actions-restore]') && state.quickActionsEditing && !state.quickActionsSaving) {
            state.homeQuickActionKeys = [...state.homeQuickActionDefaultKeys];
            state.quickActionsError = '';
            renderQuickActionsHome();
            return;
        }
        const quickActionsEdit = event.target.closest('[data-quick-actions-edit]');
        if (quickActionsEdit && canPersonalizeHome() && !state.quickActionsSaving && !state.homePreferencesSaving) {
            if (!state.quickActionsEditing) {
                state.quickActionsEditing = true;
                state.quickActionsError = '';
                rerenderPersonalizedHome();
                void requestHomePreferences();
                return;
            }

            if (!state.activeDossierId || !state.homeQuickActionsEditable || JSON.stringify(state.homeQuickActionKeys) === JSON.stringify(state.homeQuickActionInitialKeys)) {
                if (state.homeQuickActionScope && state.homeQuickActionsEditable) writeSecureMobileDraft(`home:${state.homeQuickActionScope}`, null);
                state.quickActionsEditing = false;
                state.quickActionsDialogOpen = false;
                rerenderPersonalizedHome();
                return;
            ...
            state.quickActionsError = '';
            renderQuickActionsHome();
            const savedScope = state.homeQuickActionScope;
            const savedDossierId = state.activeDossierId;
            const savedKeys = [...state.homeQuickActionKeys];
            const savedNavigationGeneration = state.navigationGeneration;
            try {
                const response = await api(`/dossiers/${savedDossierId}/home/quick-actions`, {
                    method: 'PATCH',
                    body: { action_keys: savedKeys },
                });
                if (JSON.stringify(state.homeQuickActionDrafts[savedScope]?.keys) === JSON.stringify(savedKeys)) {
                    delete state.homeQuickActionDrafts[savedScope];
                    writeSecureMobileDraft(`home:${savedScope}`, null);
                }
                if (savedScope !== state.homeQuickActionScope || savedNavigationGeneration !== state.navigationGeneration || state.route !== 'home') return;
                state.homeQuickActionCatalog = response.data.quick_action_catalog || state.homeQuickActionCatalog;
                state.homeQuickActionDefaultKeys = response.data.quick_action_default_keys || state.homeQuickActionDefaultKeys;
                ...
                if (savedScope === state.homeQuickActionScope && savedNavigationGeneration === state.navigationGeneration && state.route === 'home') {
                    state.quickActionsSaving = false;
                    renderQuickActionsHome();
                }
            }
        ...
        if (documentFolderMenuToggle) {
            let menu = app.querySelector('[data-mobile-document-folder-menu]');
            const opening = menu?.hidden !== false;
            if (opening && !state.documentFolderTreeComplete) {
                const scope = mobileDataScope();
                const navigationGeneration = state.navigationGeneration;
                documentFolderMenuToggle.disabled = true;
                try {
                    const personal = state.routeParams?.scope === 'personal';
                    const folderId = Number(state.routeParams?.folder) || 0;
                    const endpoint = personal ? '/personal-documents' : `/dossiers/${state.activeDossierId}/documents`;
                    const response = await api(`${endpoint}?folder_id=${folderId}&per_page=1&tree_scope=all`);
                    if (scope !== mobileDataScope() || state.route !== 'documents' || navigationGeneration !== state.navigationGeneration) return;
                    state.documentFolders = response.meta?.document_folders || state.documentFolders;
                    state.documentFolderTreeComplete = true;
                    const context = app.querySelector('.mobile-app-document-folder-context');
                    if (context) context.innerHTML = documentFolderSelectorMarkup(state.documentFolders, folderId, state.routeParams?.scope === 'all', personal);
                    menu = app.querySelector('[data-mobile-document-folder-menu]');
                } catch (error) {
                    showQuickCreationNotice(error?.message || 'Chargement des dossiers impossible.');
                    return;
                } finally { documentFolderMenuToggle.disabled = false; }
            }
            if (menu) menu.hidden = !opening;
            app.querySelector('[data-mobile-document-folder-menu-toggle]')?.setAttribute('aria-expanded', opening ? 'true' : 'false');
        ...
        if (documentFolderOption) {
            ...
            const currentScope = ['all', 'personal', 'recent'].includes(searchForm?.dataset.documentSearchScope)
        ...
        if (event.target.closest('[data-hr-clear-status]')) { await navigate(state.route, { ...state.routeParams, status: 'all' }); return; }
        if (event.target.closest('[data-hr-overview-toggle]')) {
            const button = event.target.closest('[data-hr-overview-toggle]');
            const body = app.querySelector('[data-hr-overview-body]');
            body.hidden = !body.hidden; button.setAttribute('aria-expanded', String(!body.hidden));
            if (!body.hidden) await loadHrOverview(); return;
        }
        if (event.target.closest('[data-hr-overview-retry]')) { await loadHrOverview(); return; }
        const overviewMonth = event.target.closest('[data-hr-overview-month]');
        if (overviewMonth && !overviewMonth.disabled) { await loadHrOverview(overviewMonth.dataset.hrOverviewMonth, Number(overviewMonth.dataset.hrOverviewPage) || 1); return; }
        const overviewLink = event.target.closest('[data-hr-overview-route]');
        if (overviewLink) {
            const filters = { expense_pending: 'submitted', expense_approved: 'approved_unpaid', expense_paid: 'paid', expense_drafts: 'draft', absence_requests: 'submitted' };
            await navigate(overviewLink.dataset.hrOverviewRoute, { year: 'all', status: filters[overviewLink.dataset.hrOverviewFilter] || 'all' }); return;
        }
        const overviewDraft = event.target.closest('[data-hr-resume-draft]');
        if (overviewDraft) { await navigate('expense_resume', { reportId: Number(overviewDraft.dataset.hrResumeDraft) }); return; }
        const agendaAbsence = event.target.closest('[data-hr-agenda-absence]');
        if (agendaAbsence) { await openHrLinkedDetail('absence', Number(agendaAbsence.dataset.hrAgendaAbsence)); return; }
        const pendingMore = event.target.closest('[data-hr-pending-more]');
        if (pendingMore && !pendingMore.disabled) {
            pendingMore.disabled = true;
            try { await loadHrPendingValidations(true); await renderHrPendingValidations(); }
            catch (error) { showQuickCreationNotice(error?.message || 'La suite ne peut pas être chargée.', 6000); pendingMore.disabled = false; }
            return;
        }

        const hrTransition = event.target.closest('[data-hr-transition]');
        if (hrTransition && !state.readOnly) { await performHrTransition(hrTransition); return; }
        ...
        if (hrRejectConfirm && !state.readOnly) { await performHrTransition(hrRejectConfirm, true); return; }
        ...
        if (preset) {
            ...
            const period = JSON.parse(form.dataset.periods || '{}')[value];
            setMobileReportDate(form.elements.period_start, period?.start || `${today.slice(0, 4)}-${value === 'year' ? '01' : today.slice(5, 7)}-01`);
            setMobileReportDate(form.elements.period_end, period?.end || today);
            syncMobileReportFields(form);
        ...
        const reportRegenerate = event.target.closest('[data-report-regenerate]');
        if (reportRegenerate && !reportRegenerate.disabled) {
            reportRegenerate.disabled = true;
            const scope = mobileDataScope();
            try {
                await api(`/dossiers/${state.activeDossierId}/reports/${Number(reportRegenerate.dataset.reportRegenerate)}/regenerate`, {
                    method: 'POST', body: {}, expectedIdentityScope: scope,
                    idempotencyKey: reportRegenerate.dataset.retryKey ||= crypto.randomUUID(),
                });
                if (scope === mobileDataScope() && state.route === 'reports') {
                    state.routeParams = {};
                    await renderReportForm('Génération relancée avec les mêmes paramètres.');
                }
            } catch (error) { showQuickCreationNotice(error?.message || 'La relance a échoué. Réessayez.', 6000); }
            finally { if (reportRegenerate.isConnected) reportRegenerate.disabled = false; }
            return;
        }
        if (event.target.closest('[data-report-refresh-retry]')) { await renderReportForm(); return; }
        ...
        if (routeButton) {
            event.preventDefault();
            if (routeButton.closest('.mobile-app-shell > .mobile-app-nav') || routeButton.classList.contains('mobile-app-topbar__brand')) {
                if (!await flushMobileNotes()) return;
                state.routeHistory = [];
                const route = routeButton.dataset.mobileRoute;
                await navigate(route, state.mainTabParams[route] || {}, false);
                return;
            }
            if (routeButton.hasAttribute('data-mobile-retry-params')) {
                await navigate(routeButton.dataset.mobileRoute, JSON.parse(routeButton.dataset.mobileRetryParams || '{}'), false);
                return;
            }
        ...
        if (event.target.closest('[data-mobile-update]')) {
            if (iosRuntime) {
                ...
                if (/^https:\/\/(testflight\.apple\.com|apps\.apple\.com)\//i.test(updateUrl)) {
                    if (window.MaisonPiloteNative?.openExternal) window.MaisonPiloteNative.openExternal(updateUrl);
                    else window.open(updateUrl, '_blank', 'noopener,noreferrer');
                } else {
                    const message = 'Le lien de mise à jour de ce canal n’est pas encore disponible. Réessayez ultérieurement.';
                    if (state.route === 'about') {
                        state.updateCheckError = message;
                        renderAbout();
                    } else {
                        renderMandatoryUpdate();
    ...
    app.addEventListener('input', (event) => {
        if (event.target.matches('#mobile-message-body')) rememberMessageDraft();
        ...
        if (noteBody instanceof HTMLTextAreaElement) {
            ...
            writeSecureMobileDraft(`note:${mobileDraftScope()}:${scope}`, state.notesDrafts[scope]);
        ...
        if (documentSearch instanceof HTMLInputElement) {
            ...
            const scope = ['all', 'personal', 'recent'].includes(form?.dataset.documentSearchScope)
                ? form.dataset.documentSearchScope
                : 'current';
            const folderId = Math.max(0, Number(form?.dataset.documentFolderId) || 0);
    ...
    app.addEventListener('submit', async (event) => {
        ...
        if (search) {
            ...
            if (search.dataset.mobileSearch === 'documents') {
                ...
                if (['all', 'personal', 'recent'].includes(search.dataset.documentSearchScope)) {
        ...
        if (messageForm) {
            ...
            const draftKey = messageDraftKey();
            const submittedDraft = state.messageDrafts[draftKey];
            button.disabled = true;
            ...
            try {
                ...
                if (state.messageDrafts[draftKey] === submittedDraft) {
                    delete state.messageDrafts[draftKey];
                    writeSecureMobileDraft(draftKey, null);
                }
                if (requestSequence === state.messageRequestSequence && state.route === 'messages') {
                    restoreMessageDraft();
                    updateMessageConversation(response.data || {}, true);
                ...
                showQuickCreationNotice(error?.message || 'Envoi impossible. Votre brouillon est conservé.');
            } finally {
                state.messageSendInFlight = false;
                if (button.isConnected) { button.disabled = state.readOnly; button.setAttribute('aria-label', 'Envoyer le message'); }
            }
            return;
        }
    ...
    app.addEventListener('change', async (event) => {
        const homeNewsToggle = event.target.closest('[data-mobile-home-news-enabled]');
        if (homeNewsToggle) {
            const value = homeNewsToggle.checked;
            homeNewsToggle.checked = state.homePreferences?.news_enabled === true;
            await requestHomePreferences(value);
            return;
        }
        ...
        if (notesTab instanceof HTMLSelectElement) {
            if (!await flushMobileNotes()) {
                notesTab.value = state.notesActiveTab;
                return false;
            }
        ...
        if (event.target.matches('[data-report-history-filter]')) { await navigate('reports', { status: event.target.value }, false); return; }
        if (event.target.matches('[data-mobile-form="report"] input[type="date"]')) syncMobileReportFields(event.target.closest('form'));
        const hrPageSelector = event.target.closest('[data-mobile-hr-page-selector]');
        if (hrPageSelector instanceof HTMLSelectElement) {
            rememberHrSection(hrPageSelector.value);
        ...
        if (absenceFilter instanceof HTMLSelectElement) {
            ...
            await navigate('absences', { ...state.routeParams, absenceId: 0, employeeId, year }, false);
            return;
        }
        ...
        if (expenseFilter) {
            ...
            await navigate('expenses', { ...state.routeParams, reportId: 0, employeeId, year }, false);
            return;
        }
    ...
    app.addEventListener('input', (event) => {
        ...
        if (hrRejectReason instanceof HTMLTextAreaElement) {
            ...
            if (confirm instanceof HTMLButtonElement) confirm.disabled = state.readOnly || confirm.dataset.hrBusy === 'true' || confirm.dataset.hrBlocked === 'true' || hrRejectReason.value.trim() === '';
            return;
        }
    ...
    app.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'Escape' && app.querySelector('[data-mobile-user-menu]:not([hidden]), [data-mobile-dossier-menu]:not([hidden]):not([data-mobile-document-folder-menu])')) {
            event.preventDefault(); closeHeaderMenus({ focus: true }); return;
        }
        const headerMenu = event.target.closest('[data-mobile-user-menu], [data-mobile-dossier-menu]:not([data-mobile-document-folder-menu])');
        if (headerMenu && !event.target.matches('input, textarea') && ['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(event.key)) {
            const buttons = [...headerMenu.querySelectorAll('button:not(:disabled)')].filter((button) => button.getClientRects().length);
    ...
    async function openMobileDeepLink(value) {
        if (openSignatureExperience(value)) return true;
        let pathname = '';
        ...
        if (siteLogin) {
            ...
            return state.authenticated && !state.pendingSiteLogin;
        }
        const cameleonNotification = cameleonNotificationTargetFromUrl(value);
        if (cameleonNotification) {
            try {
                return await startCameleonSession(cameleonNotification.userId, cameleonNotification.destination, cameleonNotification.params);
            } catch (error) {
                renderError(error, state.route);
            }
            return false;
        ...
        const mapping = [
            ['assistant-vocal', 'assistant'], ['billing', 'billing'],
        ...
        return await navigate(match?.[1] || profileLandingRoute(), { ...routeParams, ...(match?.[1] === 'assistant' && listen ? { listen: true } : {}) });
    }
    root.querySelector('[data-emulator-open-link]').addEventListener('click', async () => {
        try { await openMobileDeepLink(root.querySelector('[data-emulator-deep-link]').value); }
        catch (error) { renderError(error, state.route); }
    });

    let nativeNavigationChain = Promise.resolve();
    const nativeNavigationInFlight = new Set();
    function acceptNativeNavigation(detail) {
        const requestId = String(detail?.request_id || '');
        const url = String(detail?.url || detail?.deep_link || detail?.data?.deep_link || '');
        if (requestId && nativeNavigationInFlight.has(requestId)) return;
        if (requestId) nativeNavigationInFlight.add(requestId);
        nativeNavigationChain = nativeNavigationChain.then(async () => {
            if (!state.authenticated || !state.bootstrap || state.biometricLocked || state.applicationBackgrounded) return;
            const identity = Number(state.bootstrap.profile?.owner_user_id || state.bootstrap.profile?.id) || 0;
            let succeeded;
            if (detail?.opened_by_user === false) {
                if (state.route === 'notifications') await renderNotifications();
                succeeded = true;
            } else {
                succeeded = url ? await openMobileDeepLink(url) : await navigate('notifications');
            }
            if (succeeded === true && identity === Number(state.bootstrap?.profile?.owner_user_id || state.bootstrap?.profile?.id)) {
                if (requestId) window.MaisonPiloteNative?.navigation?.acknowledge?.(requestId, 'completed');
            }
        }).catch((error) => {
            showQuickCreationNotice(error?.message || 'Ouverture impossible. Le lien sera proposé de nouveau.');
        }).finally(() => nativeNavigationInFlight.delete(requestId));
    }
    ...
    window.addEventListener('online', () => {
        window.MaisonPiloteNative?.navigation?.retry?.();
        if (state.route === 'notes') void flushMobileNotes();
    });
    window.setInterval(() => { void refreshNotificationBadge(false); }, 60000);
    window.addEventListener('resize', syncMobileKeyboardLayout);
    window.visualViewport?.addEventListener('resize', syncMobileKeyboardLayout);
    app.addEventListener('focusin', () => window.requestAnimationFrame(syncMobileKeyboardLayout));
    app.addEventListener('focusout', () => window.setTimeout(() => {
        syncMobileKeyboardLayout();
        if (!document.activeElement?.closest('[data-mobile-user-menu], [data-mobile-dossier-menu], [data-mobile-user-menu-toggle], [data-mobile-dossier-menu-toggle]')) closeHeaderMenus();
    }, 0));
    window.addEventListener('beforeunload', (event) => {
        if (Object.keys(state.notesDrafts).length) { event.preventDefault(); event.returnValue = ''; }
    });
    ...
    window.addEventListener('resize', () => {
        ...
        window.requestAnimationFrame(() => { syncHeaderDisplayName(); syncDossierLogoAlignment(); });
    });
    ...
    window.addEventListener('maisonpilote:native-share-inbox', (event) => {
        ...
        nativeShareInboxBatches.forEach((batch) => {
            if (batch.intakeResult && !displayedShareIntakeResults.has(batch.id)) {
                displayedShareIntakeResults.add(batch.id);
                showMobileShareIntakeResult(batch.intakeResult);
            }
        });
    ...
    window.addEventListener('maisonpilote:deep-link', (event) => acceptNativeNavigation(typeof event.detail === 'string' ? { url: event.detail } : event.detail));
    ...
    window.addEventListener('maisonpilote:push-notification', (event) => acceptNativeNavigation(event.detail));
