...
if (root) {
    ...
    function preparePaginatedList(key, url, response, items, render) {
        state.paginatedLists[key] = { url, items, render, pagination: response.meta?.pagination, scope: mobileDataScope(), busy: false };
    ...
    async function loadMoreMobileItems(key, button) {
        ...
        try {
            ...
            if (state.paginatedLists[key] !== list || list.scope !== mobileDataScope() || state.route !== key) return;
    ...
    async function loadDocumentSearch(query, scope, requestSequence) {
        ...
        try {
            ...
            results.innerHTML = preparePaginatedList('documents', `${endpoint}?${parameters}`, response, response.data || [], (items) => documentListMarkup(items, childFolders, allFolders || personalDocuments, query.length > 0, personalDocuments));
    ...
    async function renderDocuments(params = {}) {
        ...
        persistNavigation();
        const list = preparePaginatedList('documents', `${endpoint}?${parameters}`, response, items, (rows) => documentListMarkup(rows, childFolders, allFolders || personalDocuments, search.length > 0, personalDocuments));
